...
class Quest(JQuest):
    ...
    def onEvent(self, event, st):
        ...
        if event == "class_change_20":
            if ClassId in [ClassId.elvenKnight]:
                if Level <= 39:
                    if (
                        st.getQuestItemsCount(MARK_OF_DUTY_ID) == 0
                        or st.getQuestItemsCount(MARK_OF_LIFE_ID) == 0
                        or st.getQuestItemsCount(MARK_OF_HEALER_ID) == 0
                    ):
        ...
        if event == "class_change_21":
            if ClassId in [ClassId.elvenKnight]:
                if Level <= 39:
                    if (
                        st.getQuestItemsCount(MARK_OF_CHALLENGER_ID) == 0
                        or st.getQuestItemsCount(MARK_OF_LIFE_ID) == 0
                        or st.getQuestItemsCount(MARK_OF_DUELIST_ID) == 0
                    ):
        ...
        if event == "class_change_5":
            if ClassId in [ClassId.knight]:
                if Level <= 39:
                    if (
                        st.getQuestItemsCount(MARK_OF_DUTY_ID) == 0
                        or st.getQuestItemsCount(MARK_OF_TRUST_ID) == 0
                        or st.getQuestItemsCount(MARK_OF_HEALER_ID) == 0
                    ):
            ...
            if ClassId in [ClassId.knight]:
                if Level <= 39:
                    if (
                        st.getQuestItemsCount(MARK_OF_DUTY_ID) == 0
                        or st.getQuestItemsCount(MARK_OF_TRUST_ID) == 0
                        or st.getQuestItemsCount(MARK_OF_WITCHCRAFT_ID) == 0
                    ):
        ...
        if event == "class_change_8":
            if ClassId in [ClassId.rogue]:
                if Level <= 39:
                    if (
                        st.getQuestItemsCount(MARK_OF_SEEKER_ID) == 0
                        or st.getQuestItemsCount(MARK_OF_TRUST_ID) == 0
                        or st.getQuestItemsCount(MARK_OF_SEARCHER_ID) == 0
                    ):
            ...
            if ClassId in [ClassId.rogue]:
                if Level <= 39:
                    if (
                        st.getQuestItemsCount(MARK_OF_SEEKER_ID) == 0
                        or st.getQuestItemsCount(MARK_OF_TRUST_ID) == 0
                        or st.getQuestItemsCount(MARK_OF_SAGITTARIUS_ID) == 0
                    ):
            ...
            if ClassId in [ClassId.warrior]:
                if Level <= 39:
                    if (
                        st.getQuestItemsCount(MARK_OF_CHALLENGER_ID) == 0
                        or st.getQuestItemsCount(MARK_OF_TRUST_ID) == 0
                        or st.getQuestItemsCount(MARK_OF_CHAMPION_ID) == 0
                    ):
    ...
    def onTalk(Self, npc, player):
        ...
        if (
            npcId == GRAND_MASTER_HANNAVALT
            or GRAND_MASTER_SIRIA
            or GRAND_MASTER_BLACKBIRD
            or GRAND_MASTER_SEDRICK
            or GRAND_MASTER_MARCUS
            and Race in [Race.elf, Race.human]
        ):
            if ClassId in [ClassId.elvenKnight]:
                st.setState(STARTED)
                return "30109-01.htm"
