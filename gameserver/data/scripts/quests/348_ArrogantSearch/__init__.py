...
class Quest(JQuest):
    ...
    def onTalk(self, npc, player):
        ...
        npcId = npc.getNpcId()
        id = st.getState()
        if npcId != HANELLIN and id != STARTED:
            return htmltext
        ...
        if npcId == HANELLIN:
            if id == CREATED:
                ...
                if st.getQuestItemsCount(BLOODED_FABRIC) == 1:
                    ...
                    # Player abandoned in the middle of last dialog...repeat the dialog.
                        ...
                        htmltext = (
                            "30864-02.htm"  # begin the dialog which will set cond=2
                        )
            # Has returned before getting the powerstone
                ...
                cond == 5
                and st.getQuestItemsCount(BOOK_OF_SAINT)
                + st.getQuestItemsCount(BLOOD_OF_SAINT)
                + st.getQuestItemsCount(BRANCH_OF_SAINT)
                < 3
            ):
                htmltext = "30864-05.htm"  # Repeat: Go get the 3 sacred relics
            elif cond == 5:
                htmltext = (
                    "30864-06.htm"  # All relics collected!...Get me antidotes & healing
                )
                ...
                cond == 22
                and st.getQuestItemsCount(ANTIDOTE) < 5
                and st.getQuestItemsCount(HEALING_POTION) < 1
            ):
                ...
                cond == 27
                and st.getInt(BLOODY_OWNERS[GUSTAV_ATHEBALDT][1])
                + st.getInt(BLOODY_OWNERS[HARDIN][1])
                + st.getInt(BLOODY_OWNERS[HEINE][1])
                < 3
            ):
        ...
        # Other NPCs follow:
            ...
            if npcId in list(ARK_OWNERS.keys()):
                ...
                if st.getQuestItemsCount(ARK_OWNERS[npcId][0]) == 1:
                    ...
                    st.addRadar(
                        ARK_OWNERS[npcId][5][0],
                        ARK_OWNERS[npcId][5][1],
                        ARK_OWNERS[npcId][5][2],
                    )
                ...
                # if you do not have the key (first meeting)
                if (
                    st.getQuestItemsCount(FIRST_KEY_OF_ARK) == 0
                    and st.getInt("angelKillerIsDefeated") == 0
                ):
                    st.addSpawn(ANGEL_KILLER, 120000)
                    htmltext = "30980-01.htm"
                elif (
                    st.getQuestItemsCount(FIRST_KEY_OF_ARK) == 0
                    and st.getInt("angelKillerIsDefeated") == 1
                ):
    ...
    def onAttack(self, npc, player, damage, isPet):
        ...
        if npcId in list(ATTACK_DROPS_24.keys()):
            ...
            if (
                st.getInt("cond") == cond
                and st.getRandom(1000) < ATTACK_DROPS_24[npcId][3]
                and st.getQuestItemsCount(ATTACK_DROPS_24[npcId][4]) > 0
            ):  # Attack drops are low chance
            ...
            if (
                st.getInt("cond") == cond
                and st.getRandom(1000) < ATTACK_DROPS_25[npcId][3]
                and st.getQuestItemsCount(ATTACK_DROPS_25[npcId][4]) > 0
            ):  # Attack drops are low chance
    ...
    def onKill(self, npc, player, isPet):
        ...
        if npcId in list(DROPS.keys()):
            ...
            if (
                st.getInt("cond") == cond
                and st.getQuestItemsCount(DROPS[npcId][1]) < DROPS[npcId][2]
                and st.getRandom(100) < DROPS[npcId][3]
                and (DROPS[npcId][4] == 0 or st.getQuestItemsCount(DROPS[npcId][4]) > 0)
            ):
        ...
        if npcId in list(DROPS_29.keys()):
            ...
            if (
                st.getInt("cond") == cond
                and st.getQuestItemsCount(DROPS_29[npcId][1]) < DROPS[npcId][2]
                and st.getRandom(100) < DROPS[npcId][3]
                and (DROPS[npcId][4] == 0 or st.getQuestItemsCount(DROPS[npcId][4]) > 0)
            ):
