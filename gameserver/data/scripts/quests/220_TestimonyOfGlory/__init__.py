...
class Quest(JQuest):
    ...
    def onEvent(self, event, st):
        ...
        # Everything bellow here is for condition 4 and 5
            ...
            if (
                st.getQuestItemsCount(SCEPTER_OF_BREKA)
    ...
    def onTalk(self, npc, player):
        ...
        if id == CREATED:  # Check if is starting the quest
            ...
            st.set("id", "0")
            if npcId == 30514:
                if player.getRace().ordinal() == 3 and player.getClassId().getId() in [
                    45,
                    47,
                    50,
                ]:
            ...
            # Condition Oriented code for optimal performance
                ...
                if (
                    npcId == 30514
                    and st.getQuestItemsCount(VOKIYANS_ORDER1)
                    and (
                        st.getQuestItemsCount(MANASHEN_SHARD) < 10
                ...
                # Begin of the cross compatible code
                    ...
                    if (
                        st.getQuestItemsCount(VOKIYANS_ORDER1)
                        and st.getQuestItemsCount(MANASHEN_SHARD) == 10
                        and st.getQuestItemsCount(TYRANT_TALON) == 10
                        and st.getQuestItemsCount(GUARDIAN_BASILISK_FANG) == 10
                    ):
                        st.set("cond", "2")
                    elif st.getQuestItemsCount(
                        NECKLACE_OF_AUTHORITY
                    ) and st.getQuestItemsCount(VOKIYANS_ORDER2):
                        st.set("cond", "3")
                    elif st.getQuestItemsCount(
                        NECKLACE_OF_AUTHORITY
                    ) and st.getQuestItemsCount(CHIANTAS_ORDER1):
                        st.set("cond", "4")
                    elif st.getQuestItemsCount(NECKLACE_OF_AUTHORITY) and (
                        st.getQuestItemsCount(CHIANTAS_ORDER2)
                        or (
                        ...
                        NECKLACE_OF_AUTHORITY
                    ) and st.getQuestItemsCount(CHIANTAS_ORDER3):
                        if (
                            st.getQuestItemsCount(TAMLIN_ORC_SKULL) == 20
                            and st.getQuestItemsCount(TIMAK_ORC_HEAD) == 20
                        ):
                            st.set("cond", "7")
                        else:
                            st.set("cond", "6")
                ...
                if npcId == 30514:
                    ...
                    npcId == 30642
                    and st.getQuestItemsCount(NECKLACE_OF_AUTHORITY)
                    and st.getQuestItemsCount(VOKIYANS_ORDER2)
                ):
                ...
                if npcId == 30514 and st.getQuestItemsCount(VOKIYANS_ORDER2):
                    ...
                    if st.getQuestItemsCount(MANAKIAS_LETTER1):
                        ...
                        st.addSpawn(27080, 80117, 120039, -2259)
                        st.addSpawn(27081, 80058, 120038, -2259)
                    elif st.getQuestItemsCount(PASHIKAS_HEAD) and st.getQuestItemsCount(
                        VULTUS_HEAD
                    ):
                ...
                if (
                    npcId == 30642
                    and st.getQuestItemsCount(TAMLIN_ORC_SKULL) == 20
                    and st.getQuestItemsCount(TIMAK_ORC_HEAD) == 20
                ):
                ...
                if (
                    npcId == 30571
                    and st.getQuestItemsCount(TANAPIS_ORDER1)
                    and st.getQuestItemsCount(SCEPTER_OF_TANTOS)
                ):
        ...
        if cond == 1 and npcId in list(DROPLIST_COND_1.keys()):
            ...
            if st.getQuestItemsCount(required) and count < maxquanty:
                ...
                if count + 1 == maxquanty:  # Check if got enough number of items
                    if (
                        st.getQuestItemsCount(MANASHEN_SHARD) == maxquanty
                        and st.getQuestItemsCount(TYRANT_TALON) == maxquanty
                        and st.getQuestItemsCount(GUARDIAN_BASILISK_FANG) == maxquanty
                    ):
                        st.set("cond", "2")
                        st.playSound("Itemsound.quest_middle")
                    else:
            ...
            if st.getQuestItemsCount(required) and count < maxquanty:
                if npcId == 20234:
                    if st.getRandom(4) <= 2:
                        ...
                        if (
                            count + 1 == maxquanty
                        ):  # Check if got enough number of items
                            st.playSound("Itemsound.quest_middle")
                        else:
                            st.playSound("Itemsound.quest_itemget")
                else:
                    ...
                    if count + 1 == maxquanty:  # Check if got enough number of items
                        if npcId in [27080, 27081]:
                            if st.getQuestItemsCount(
                                PASHIKAS_HEAD
                            ) and st.getQuestItemsCount(VULTUS_HEAD):
            ...
            if st.getQuestItemsCount(required) and count < maxquanty:
                if st.getRandom(100) < chance:
                    ...
                    if count + 1 == maxquanty:  # Check if got enough number of items
                        if (
                            st.getQuestItemsCount(TIMAK_ORC_HEAD) == maxquanty
                            and st.getQuestItemsCount(TAMLIN_ORC_SKULL) == maxquanty
                        ):
                            st.set("cond", "7")
                            st.playSound("Itemsound.quest_middle")
                        else:
            ...
            cond == 9
            and st.getQuestItemsCount(TANAPIS_ORDER1)
            and st.getQuestItemsCount(SCEPTER_OF_TANTOS) == 0
        ):
            if npcId in [20778, 20779]:
                ...
                return (
                    "Revenant of Tantos Chief has spawned at X=11839 Y=-106261 Z=-3550"
                )
                # Alternate coord. set:
