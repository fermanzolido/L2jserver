    ...
    def onTalk(Self, npc, player):
        ...
        if npcId == 30981:  # Black Judge
            if id == CREATED:
                ...
                condition <= 13
                and condition > 9
                and st.getQuestItemsCount(MANUAL_OF_MANACLES) == 0
            ):
            ...
            if condition >= 14:
                if st.getQuestItemsCount(MANUAL_OF_MANACLES) == 1:
                    if (
                        st.getQuestItemsCount(SILVER_NUGGET) < 10
                        or st.getQuestItemsCount(STEEL) < 5
                        or st.getQuestItemsCount(ADAMANTINE_NUGGET) < 2
                        or st.getQuestItemsCount(COKES) < 10
                        or st.getQuestItemsCount(BLACKSMITHS_FRAME) < 1
                    ):
                        htmltext = "30300-02.htm"
                    elif (
                        st.getQuestItemsCount(SILVER_NUGGET) >= 10
                        and st.getQuestItemsCount(STEEL) >= 5
                        and st.getQuestItemsCount(ADAMANTINE_NUGGET) >= 2
                        and st.getQuestItemsCount(COKES) >= 10
                        and st.getQuestItemsCount(BLACKSMITHS_FRAME) >= 1
                    ):
                        htmltext = "30300-02.htm"
                        st.set("cond", "15")
                        st.takeItems(MANUAL_OF_MANACLES, 1)
                    ...
                    st.getQuestItemsCount(PENITENTS_MANACLES1)
                    or st.getQuestItemsCount(PENITENTS_MANACLES)
                    or st.getQuestItemsCount(PENITENTS_MANACLES2)
                ):
