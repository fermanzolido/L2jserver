...
class Quest(JQuest):
    ...
    def giveHallishaMark(self, st2):
        if st2.getInt("spawned") == 0:
            if st2.getQuestItemsCount(self.Items[3]) >= 700:
                ...
                self.AutoChat(
                    Archon,
                    self.Text[13].replace("PLAYERNAME", st2.getPlayer().getName()),
                )
                Archon.addDamageHate(st2.getPlayer(), 0, 99999)
                Archon.getAI().setIntention(
                    CtrlIntention.AI_INTENTION_ATTACK, st2.getPlayer(), None
                )
            else:
    ...
    def onAdvEvent(self, event, npc, player):
        ...
        if not st:
            return
        htmltext = (
            ""  # simple initialization...if none of the events match, return nothing.
        )
        cond = st.getInt("cond")
        id = st.getInt("id")
        player = st.getPlayer()
        if event == "accept":
            ...
            if st.getInt("Quest0") == 0:
                ...
                if st.getRandom(2) == 0:
                    self.AutoChat(
                        npc, self.Text[9].replace("PLAYERNAME", player.getName())
                    )
                else:
                    self.AutoChat(
                        npc, self.Text[10].replace("PLAYERNAME", player.getName())
                    )
            return
            ...
            if (
                st.getInt("Quest0") == 1
                or st.getInt("Quest0") == 2
                or st.getInt("Quest1") > 3
            ):
                st.set("Quest0", "0")
                if st.getInt("Quest0") == 1:
                    self.AutoChat(
                        npc, self.Text[11].replace("PLAYERNAME", player.getName())
                    )
                else:
                    self.AutoChat(
                        npc, self.Text[12].replace("PLAYERNAME", player.getName())
                    )
                npc.reduceCurrentHp(9999999, npc)
                self.DeleteSpawn(st, npc.getObjectId())
            else:
                ...
                if npc.getNpcId() == self.Mob[2]:
                    ...
                    if st == st2:
                        ...
                        if st.getInt("Quest0") == 1:
                            self.AutoChat(
                                npc,
                                self.Text[16].replace("PLAYERNAME", player.getName()),
                            )
                        if st.getInt("Quest0") > 15:
                            st.set("Quest0", "1")
                            self.AutoChat(
                                npc,
                                self.Text[17].replace("PLAYERNAME", player.getName()),
                            )
                            npc.reduceCurrentHp(9999999, npc)
                    ...
                    if player.getClassId().getId() == QuestClass[int(q) - 70]:
                        if st1.getInt("cond") == 15:
                            ...
                            self.AutoChat(
                                ...
                                st1.getQuest()
                                .Text[4]
                                .replace("PLAYERNAME", st1.getPlayer().getName()),
                            )
                ...
                if st2:
                    if not player.isInParty():
                        if st == st2:
                            self.AutoChat(
                                npc,
                                self.Text[12].replace("PLAYERNAME", player.getName()),
                            )
