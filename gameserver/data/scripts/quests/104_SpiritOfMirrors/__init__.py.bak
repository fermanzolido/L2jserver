...
class Quest(JQuest):
    ...
    def onTalk(self, npc, player):
        ...
        if npcId == 30017 and st.getInt("cond") == 0 and st.getInt("onlyone") == 0:
            ...
            if (
                npcId == 30017
                and st.getInt("cond")
                and st.getQuestItemsCount(GALLINS_OAK_WAND_ID) >= 1
                and not HaveAllQuestItems(st)
            ):
    ...
    def onKill(self, npc, player, isPet):
        ...
        if (
            st.getInt("cond") >= 1
            and st.getItemEquipped(7) == GALLINS_OAK_WAND_ID
            and not st.getQuestItemsCount(DROPLIST[npcId])
        ):  # (7) means weapon slot
