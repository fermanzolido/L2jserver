...
(
    ...
    VIKTORS_LETTER_ID,
    HAWKEYES_LETTER_ID,
    MYSTERIOUS_RUNESTONE_ID,
    OL_MAHUM_RUNESTONE_ID,
    TUREK_RUNESTONE_ID,
    ANT_RUNESTONE_ID,
    TURAK_BUGBEAR_RUNESTONE_ID,
    TERYS_BOX_ID,
    VIKTORS_REQUEST_ID,
    MEDUSAS_SCALES_ID,
    SILENS_RUNESTONE_ID,
        ...
        if st.getQuestItemsCount(required) and count < maxqty:
            if st.getRandom(100) < chance:
                ...
                if count + 1 == maxqty:
                    ...
                    if cond == 4:
                        if (
                            st.getQuestItemsCount(OL_MAHUM_RUNESTONE_ID)
                            + st.getQuestItemsCount(TUREK_RUNESTONE_ID)
                            + st.getQuestItemsCount(ANT_RUNESTONE_ID)
                            + st.getQuestItemsCount(TURAK_BUGBEAR_RUNESTONE_ID)
                            == 4
                        ):
                        ...
                        if (
                            st.getQuestItemsCount(ABYSS_RUNESTONE1_ID)
                            + st.getQuestItemsCount(ABYSS_RUNESTONE2_ID)
                            + st.getQuestItemsCount(ABYSS_RUNESTONE3_ID)
                            + st.getQuestItemsCount(ABYSS_RUNESTONE4_ID)
                            == 4
                        ):
                            st.set("cond", str(cond + 1))
                        return
