...
class Quest(JQuest):
    ...
    def onTalk(self, npc, player):
        ...
        if npcId == 30624 and st.getInt("cond") == 0:
            ...
            and st.getQuestItemsCount(MASONS_LETTER)
        ):
            htmltext = "30624-09.htm"
        elif (
            npcId == 30624
            and st.getInt("cond") == 5
            and st.getQuestItemsCount(ASCALONS_LETTER2)
        ):
            ...
            and st.getQuestItemsCount(WHITE_ROSE_INSIGNIA)
        ):
            htmltext = "30624-12.htm"
        elif (
            npcId == 30624
            and st.getInt("cond") == 8
            and st.getQuestItemsCount(GROOTS_LETTER)
        ):
            htmltext = "30624-13.htm"
        elif (
            npcId == 30624
            and st.getInt("cond") == 9
            and st.getQuestItemsCount(ASCALONS_LETTER3)
        ):
            htmltext = "30624-15.htm"
        elif (
            npcId == 30624
            and st.getInt("cond") == 14
            and st.getQuestItemsCount(MOUENS_LETTER)
        ):
            ...
            and st.getQuestItemsCount(ASCALONS_LETTER1)
        ):
            ...
            and st.getInt("cond") >= 5
            and (
                st.getQuestItemsCount(ASCALONS_LETTER2)
                or st.getQuestItemsCount(WHITE_ROSE_INSIGNIA)
                or st.getQuestItemsCount(GROOTS_LETTER)
                or st.getQuestItemsCount(ASCALONS_LETTER3)
                or st.getQuestItemsCount(MOUENS_ORDER1)
                or st.getQuestItemsCount(MOUENS_ORDER2)
                or st.getQuestItemsCount(MOUENS_LETTER)
            )
        ):
            ...
            npcId == 30093
            and st.getInt("step") >= 9
            and (
                st.getQuestItemsCount(ASCALONS_LETTER3)
                or st.getQuestItemsCount(MOUENS_ORDER1)
                or st.getQuestItemsCount(MOUENS_ORDER2)
                or st.getQuestItemsCount(MOUENS_LETTER)
            )
            ...
            st.takeItems(
                LETO_LIZARDMAN_FANG1, st.getQuestItemsCount(LETO_LIZARDMAN_FANG1)
            )
            st.playSound("Itemsound.quest_middle")
            st.set("cond", "14")
    ...
    def onKill(self, npc, player, isPet):
        ...
        if (
            cond == step
            and st.getQuestItemsCount(itemid) < maxcount
