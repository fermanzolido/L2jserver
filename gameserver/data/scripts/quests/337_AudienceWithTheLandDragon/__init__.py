...
(
    ...
    PORTAL_STONE,
    HERALD_OF_SLAYER,
) = list(range(3852, 3866)) + [3890]


def checkCond(st):
    if (
        st.getInt("orven") == 1
        and st.getInt("kendra") == 1
        and st.getInt("chakiris") == 1
        and st.getInt("kaiena") == 1
    ):
        st.set("all", "1")
    ...
    def onTalk(self, npc, player):
        ...
        if npcId == CHAKIRIS:
            if st.getInt("all") == 1:  # if all 4 tasks have been done
                ...
                if (
                    st.getQuestItemsCount(HAMRUT_LEG) == 0
                    or st.getQuestItemsCount(KRANROT_SKIN) == 0
                ):
        ...
        if npcId == KAIENA:
            if st.getInt("all") == 1:  # if all 4 tasks have been done
                ...
                if (
                    st.getQuestItemsCount(MARSH_STALKER_HORN) == 0
                    or st.getQuestItemsCount(MARSH_DRAKE_TALONS) == 0
                ):
        ...
        if npcId == MOKE:
            if st.getInt("all") == 2:
                ...
                if (
                    st.getQuestItemsCount(MARA_FANG) == 0
                    or st.getQuestItemsCount(FIRST_FRAGMENT_OF_ABYSS_JEWEL) == 0
                ):
        ...
        if npcId == HELTON:
            if st.getInt("all") == 2:
                ...
                if (
                    st.getQuestItemsCount(MUSFEL_FANG) == 0
                    or st.getQuestItemsCount(SECOND_FRAGMENT_OF_ABYSS_JEWEL) == 0
                ):
            ...
            if npcId == ABYSS_JEWEL1:
                if cond == 2 and st.getInt("moke") != 1:
                    if nowHp < maxHp * 0.8 and st.getInt("aspawned") != 1:
                        ...
                        nowHp < maxHp * 0.4
                        and st.getQuestItemsCount(FIRST_FRAGMENT_OF_ABYSS_JEWEL) == 0
                    ):
            ...
            if npcId == ABYSS_JEWEL2:
                if cond == 2 and st.getInt("helton") != 1:
                    if nowHp < maxHp * 0.8 and st.getInt("bspawned") != 1:
                        ...
                        nowHp < maxHp * 0.4
                        and st.getQuestItemsCount(SECOND_FRAGMENT_OF_ABYSS_JEWEL) == 0
                    ):
    ...
    def onKill(self, npc, player, isPet):
        ...
        if st:
            ...
            if cond == 1:
                if (
                    npcId == HAMRUT
                    and st.getQuestItemsCount(HAMRUT_LEG) == 0
                    and st.getInt("chakiris") == 0
                ):
                    st.giveItems(HAMRUT_LEG, 1)
                    st.playSound("ItemSound.quest_itemget")
                elif (
                    npcId == KRANROT
                    and st.getQuestItemsCount(KRANROT_SKIN) == 0
                    and st.getInt("chakiris") == 0
                ):
                    ...
                    npcId == MARSH_STALKER
                    and st.getQuestItemsCount(MARSH_STALKER_HORN) == 0
                    and st.getInt("kaiena") == 0
                ):
                    st.giveItems(MARSH_STALKER_HORN, 1)
                    st.playSound("ItemSound.quest_itemget")
                elif (
                    npcId == MARSH_DRAKE
                    and st.getQuestItemsCount(MARSH_DRAKE_TALONS) == 0
                    and st.getInt("kaiena") == 0
                ):
                    ...
                    and st.getQuestItemsCount(REMAINS_OF_SACRIFICED) == 0
                    and st.getInt("orven") == 0
                ):
                    for i in range(8):
                        st.addSpawn(SACRIFICE_OF_THE_SACRIFICED, 180000)
                elif (
                    npcId == SACRIFICE_OF_THE_SACRIFICED
                    and st.getQuestItemsCount(REMAINS_OF_SACRIFICED) == 0
                    and st.getInt("orven") == 0
                ):
                    ...
                    and st.getRandom(5) == 0
                    and st.getQuestItemsCount(TOTEM_OF_LAND_DRAGON) == 0
                    and st.getInt("kendra") == 0
                ):
                    for i in range(3):
                        st.addSpawn(HARIT_LIZARDMAN_ZEALOT, 180000)
                elif (
                    npcId == HARIT_LIZARDMAN_ZEALOT
                    and st.getQuestItemsCount(TOTEM_OF_LAND_DRAGON) == 0
                    and st.getInt("kendra") == 0
                ):
                ...
                if (
                    npcId == GUARDIAN1
                    and st.getQuestItemsCount(MARA_FANG) == 0
                    and st.getInt("moke") != 1
                ):
                    st.giveItems(MARA_FANG, 1)
                    st.playSound("ItemSound.quest_itemget")
                elif (
                    npcId == GUARDIAN2
                    and st.getQuestItemsCount(MUSFEL_FANG) == 0
                    and st.getInt("helton") != 1
                ):
                    st.giveItems(MUSFEL_FANG, 1)
                    st.playSound("ItemSound.quest_itemget")
            elif cond == 4:
                if (
                    npcId in (CAVE_MAIDEN, CAVE_KEEPER, CAVE_KEEPER1, CAVE_MAIDEN1)
                    and st.getQuestItemsCount(THIRD_FRAGMENT_OF_ABYSS_JEWEL) == 0
                    and st.getRandom(5) == 0
                ):
...
for i in list(range(3852, 3865)) + [3890]:
