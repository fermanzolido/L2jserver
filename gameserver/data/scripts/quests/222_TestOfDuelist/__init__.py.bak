...
class Quest(JQuest):
    ...
    def onTalk(self, npc, player):
        ...
        if id == COMPLETED:
            ...
            if (
                st.getQuestItemsCount(ORDER_GLUDIO)
                and st.getQuestItemsCount(ORDER_DION)
                and st.getQuestItemsCount(ORDER_GIRAN)
                and st.getQuestItemsCount(ORDER_OREN)
                and st.getQuestItemsCount(ORDER_ADEN)
            ):
                if (
                    st.getQuestItemsCount(PUNCHERS_SHARD)
                    == st.getQuestItemsCount(NOBLE_ANTS_FEELER)
                    == st.getQuestItemsCount(DRONES_CHITIN)
                    == st.getQuestItemsCount(DEADSEEKER_FANG)
                    == st.getQuestItemsCount(OVERLORD_NECKLACE)
                    == st.getQuestItemsCount(CRIMSONBINDS_CHAIN)
                    == st.getQuestItemsCount(CHIEFS_AMULET)
                    == st.getQuestItemsCount(TEMPERED_EYE_MEAT)
                    == st.getQuestItemsCount(TAMRIN_ORCS_RING)
                    == st.getQuestItemsCount(TAMRIN_ORCS_ARROW)
                    == 10
                ):
            ...
            if (
                st.getQuestItemsCount(EXCUROS_SKIN)
                == st.getQuestItemsCount(KRATORS_SHARD)
                == st.getQuestItemsCount(RAKINS_MACE)
                == st.getQuestItemsCount(GRANDIS_SKIN)
                == st.getQuestItemsCount(TIMAK_ORCS_BELT)
                > 2
            ):
