                    ...
                    if st.getInt("playing"):
                        ...
                        if chosen.count("?") >= 3:
                            ...
                            if chosen.count("?") == 3:
                                ...
                                row = col = diag = 0
                                for i in range(3):
                                    if "".join(chosen[3 * i : 3 * i + 3]).isdigit():
                                        row += 1
                                    if "".join(chosen[i:9:3]).isdigit():
                                        col += 1
                                if "".join(chosen[0:9:4]).isdigit():
                                    diag += 1
                                if "".join(chosen[2:7:2]).isdigit():
                                    diag += 1
                                if (col + row + diag) == 3:
                                    htmltext += winner
                                    ...
                                    htmltext += loser
                                    st.giveItems(
                                        REWARDS[st.getRandom(len(REWARDS))], 10
                                    )
