...
class Quest(JQuest):
    ...
    def onTalk(self, npc, player):
        ...
        npcId = npc.getNpcId()
        id = st.getState()
        if npcId != BRUNON and id != STARTED:
            return htmltext
        ...
        if npcId == BRUNON and id == CREATED:
            ...
            and st.getQuestItemsCount(CALCULATOR_Q) == 0
        ):
            ...
            and st.getQuestItemsCount(GEMSTONE_BEAST_CRYSTAL) < 10
        ):
            ...
            and st.getQuestItemsCount(CALCULATOR_Q) == 1
        ):
    ...
    def onKill(self, npc, player, isPet):
        ...
        npcId = npc.getNpcId()
        if (
            npcId == GEMSTONE_BEAST
            and st.getInt("cond") == 5
            and st.getRandom(2) == 1
            and st.getQuestItemsCount(GEMSTONE_BEAST_CRYSTAL) < 10
        ):
