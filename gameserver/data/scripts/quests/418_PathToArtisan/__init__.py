...
class Quest(JQuest):
    ...
    def onEvent(self, event, st):
        ...
        if event == "30527_1":
            ...
            if (
                st.getQuestItemsCount(PASS_1ST)
                and st.getQuestItemsCount(PASS_2ND)
                and st.getQuestItemsCount(SECRET_BOX2)
            ):
    ...
    def onTalk(self, npc, player):
        ...
        if npcId == 30527 and st.getInt("cond") == 0:
            ...
            npcId == 30527
            and st.getInt("cond")
            and st.getQuestItemsCount(SILVERYS_RING) == 1
            and (
                st.getQuestItemsCount(RATMAN_TOOTH)
                + st.getQuestItemsCount(BIG_RATMAN_TOOTH)
            )
            < 12
        ):
            ...
            and st.getInt("cond")
            and st.getQuestItemsCount(SILVERYS_RING) == 1
            and st.getQuestItemsCount(RATMAN_TOOTH) >= 10
            and st.getQuestItemsCount(BIG_RATMAN_TOOTH) >= 2
        ):
            ...
            npcId == 30527
            and st.getInt("cond")
            and st.getQuestItemsCount(PASS_1ST) == 1
        ):
            htmltext = "30527-09.htm"
        elif (
            npcId == 30317
            and st.getInt("cond")
            and st.getQuestItemsCount(KLUTOS_LETTER) == 0
            and st.getQuestItemsCount(PASS_1ST)
            and st.getQuestItemsCount(PASS_2ND) == 0
            and st.getQuestItemsCount(SECRET_BOX2) == 0
        ):
            ...
            and st.getInt("cond")
            and st.getQuestItemsCount(PASS_1ST)
            and st.getQuestItemsCount(KLUTOS_LETTER)
        ):
            ...
            and st.getInt("cond")
            and st.getQuestItemsCount(PASS_1ST)
            and st.getQuestItemsCount(FOOTPRINT)
            and st.getQuestItemsCount(SECRET_BOX1) == 0
        ):
            ...
            and st.getQuestItemsCount(PASS_1ST)
            and st.getQuestItemsCount(FOOTPRINT)
            and st.getQuestItemsCount(SECRET_BOX1)
        ):
        ...
        if npcId == 20389:
            ...
            if (
                ...
                if st.getRandom(10) < 7:
                    if (
                        st.getQuestItemsCount(RATMAN_TOOTH) == 9
                        and st.getQuestItemsCount(BIG_RATMAN_TOOTH) == 2
                    ):
            ...
            if (
                ...
                if st.getRandom(10) < 5:
                    if (
                        st.getQuestItemsCount(BIG_RATMAN_TOOTH) == 1
                        and st.getQuestItemsCount(RATMAN_TOOTH) == 10
                    ):
            ...
            if (
                st.getInt("cond")
                and st.getQuestItemsCount(FOOTPRINT) == 1
                and st.getQuestItemsCount(SECRET_BOX1) < 1
            ):
