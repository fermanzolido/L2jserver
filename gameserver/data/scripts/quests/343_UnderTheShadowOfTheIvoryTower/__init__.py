    ...
    def onEvent(self, event, st):
        ...
        if event == "30834-02.htm":
            ...
            if orbs >= 10:
                st.takeItems(ORB, 10)
                st.set("playing", "1")
                htmltext = st.showHtmlFile("30934-03.htm").replace(
                    "<msg>", start_msg[st.getRandom(len(start_msg))]
                )
            else:
                htmltext = "30934-03a.htm"
        elif event in ["1", "2", "3"]:
            if st.getInt("playing"):
                ...
                st.unset("playing")
                htmltext = (
                    st.showHtmlFile("30934-04.htm")
                    .replace("%player%", OPTIONS[player])
                    .replace("%marsha%", OPTIONS[marsha])
                    .replace("%msg%", msg[st.getRandom(len(msg))])
            ...
            if st.getInt("toss"):
                if orbs >= 10:
                    ...
                    st.set("row", str(row))
                    htmltext = (
                        st.showHtmlFile(template)
                        .replace("%toss%", TOSS[random2])
                        .replace("%msg1%", toss_msg[row - 1][0])
                        .replace("%msg2%", toss_msg[row - 1][1])
                        .replace("%orbs%", str(ORBS[row - 1]))
                        .replace("%next%", str(ORBS[row]))
                    )
                else:
                    st.unset("row")
                    htmltext = "30935-02a.htm"
            ...
            if st.getInt("row"):
                ...
                st.unset("row")
                htmltext = st.showHtmlFile("30935-06a.htm").replace(
                    "%nebulites%", str(ORBS[qty])
                )
            else:
                st.takeItems(ORB, -1)
                htmltext = "Player is cheating"
