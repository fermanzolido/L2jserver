...
class Quest(JQuest):
    ...
    def onEvent(self, event, st):
        ...
        if event == "1":
            ...
            if (
                st.getQuestItemsCount(KENDNELLS_ORDER1_ID)
                + st.getQuestItemsCount(KENDNELLS_ORDER2_ID)
                + st.getQuestItemsCount(KENDNELLS_ORDER3_ID)
                + st.getQuestItemsCount(KENDNELLS_ORDER4_ID)
                == 0
            ):
                n = st.getRandom(100)
                if n < 25:
    ...
    def onTalk(self, npc, player):
        ...
        if npcId == 30218 and st.getInt("cond") == 0 and st.getInt("onlyone") == 0:
            ...
            if st.getQuestItemsCount(KABOO_CHIEF_TORC1_ID):
                ...
                st.getQuestItemsCount(KENDNELLS_ORDER1_ID)
                or st.getQuestItemsCount(KENDNELLS_ORDER2_ID)
                or st.getQuestItemsCount(KENDNELLS_ORDER3_ID)
                or st.getQuestItemsCount(KENDNELLS_ORDER4_ID)
            ):
                ...
                if st.getInt("id") != 105:
                    ...
                    for item in range(4412, 4417):
                        st.giveItems(
                            item, int(10 * Config.RATE_QUESTS_REWARD)
                        )  # Echo crystals
                ...
                st.getQuestItemsCount(KENDNELLS_ORDER5_ID)
                or st.getQuestItemsCount(KENDNELLS_ORDER6_ID)
                or st.getQuestItemsCount(KENDNELLS_ORDER7_ID)
                or st.getQuestItemsCount(KENDNELLS_ORDER8_ID)
            ):
    ...
    def onKill(self, npc, player, isPet):
        ...
        if npcId == 27059:
            ...
            if st.getInt("cond") == 1:
                if (
                    st.getQuestItemsCount(KENDNELLS_ORDER1_ID)
                    and st.getQuestItemsCount(KABOO_CHIEF_TORC1_ID) == 0
                ):
            ...
            if st.getInt("cond") == 1:
                if (
                    st.getQuestItemsCount(KENDNELLS_ORDER5_ID)
                    and st.getQuestItemsCount(KABOO_CHIEF_TORC2_ID) == 0
                ):
            ...
            if st.getInt("cond") == 1:
                if (
                    st.getQuestItemsCount(KENDNELLS_ORDER6_ID)
                    and st.getQuestItemsCount(KABOO_CHIEF_TORC2_ID) == 0
                ):
                    st.giveItems(KABOO_CHIEF_TORC2_ID, 1)
                    st.playSound("ItemSound.quest_middle")
            ...
            if st.getInt("cond") == 1:
                if (
                    st.getQuestItemsCount(KENDNELLS_ORDER7_ID)
                    and st.getQuestItemsCount(KABOO_CHIEF_TORC2_ID) == 0
                ):
                    st.giveItems(KABOO_CHIEF_TORC2_ID, 1)
                    st.playSound("ItemSound.quest_middle")
            ...
            if st.getInt("cond") == 1:
                if (
                    st.getQuestItemsCount(KENDNELLS_ORDER8_ID)
                    and st.getQuestItemsCount(KABOO_CHIEF_TORC2_ID) == 0
                ):
                    st.giveItems(KABOO_CHIEF_TORC2_ID, 1)
                    st.playSound("ItemSound.quest_middle")
