...
# DROPLIST
DROPLIST = {
    AMBER_BASILISK: [AMBER_SCALE, 15],
    WHISPERING_WIND: [WIND_SOULSTONE, 20],
    GLASS_JAGUAR: [GLASS_EYE, 35],
    HORROR_MIST_RIPPER: [HORROR_ECTOPLASM, 15],
    SILENOS: [SILENOS_HORN, 30],
    ANT_RECRUIT: [ANT_SOLDIER_APHID, 40],
    ANT_WARRIOR_CAPTAIN: [ANT_SOLDIER_APHID, 40],
    TYRANT: [TYRANTS_CHITIN, 50],
    TYRANT_KINGPIN: [TYRANTS_CHITIN, 50],
    TURAK_BUGBEAR: [BUGBEAR_BLOOD, 25],
...
def check_ingredients(st, required):
    if st.getQuestItemsCount(AMBER_SCALE) != required:
        return 0
    if st.getQuestItemsCount(WIND_SOULSTONE) != required:
        return 0
    if st.getQuestItemsCount(GLASS_EYE) != required:
        return 0
    if st.getQuestItemsCount(HORROR_ECTOPLASM) != required:
        return 0
    if st.getQuestItemsCount(SILENOS_HORN) != required:
        return 0
    if st.getQuestItemsCount(ANT_SOLDIER_APHID) != required:
        return 0
    if st.getQuestItemsCount(TYRANTS_CHITIN) != required:
        return 0
    if st.getQuestItemsCount(BUGBEAR_BLOOD) != required:
        return 0
    return 1
...
def autochat(npc, text):
    if npc:
        npc.broadcastPacket(CreatureSay(npc.getObjectId(), 0, npc.getName(), text))
    return
...
class Quest(JQuest):
    ...
    def onAdvEvent(self, event, npc, player):
        ...
        if event == "30738-03.htm":
            ...
            if st.getQuestItemsCount(ALCHEMY_TEXT) >= 2:
                st.takeItems(ALCHEMY_TEXT, -1)
            if st.getQuestItemsCount(ALCHEMY_TEXT) == 0:
        ...
        if event == "30738-06.htm":
            if st.getQuestItemsCount(WISH_POTION):
                ...
                st.playSound("ItemSound.quest_accept")
                st.set("cond", "3")
                if st.getQuestItemsCount(ALCHEMY_TEXT) >= 1:
                    st.takeItems(ALCHEMY_TEXT, -1)
                if st.getQuestItemsCount(SECRET_BOOK) >= 1:
                    st.takeItems(SECRET_BOOK, -1)
                if st.getQuestItemsCount(POTION_RECIPE_1) >= 2:
                    st.takeItems(POTION_RECIPE_1, -1)
                if st.getQuestItemsCount(POTION_RECIPE_1) == 0:
                    st.giveItems(POTION_RECIPE_1, 1)
                if st.getQuestItemsCount(POTION_RECIPE_2) >= 2:
                    st.takeItems(POTION_RECIPE_2, -1)
                if st.getQuestItemsCount(POTION_RECIPE_2) == 0:
                    st.giveItems(POTION_RECIPE_2, 1)
                if st.getQuestItemsCount(MATILDS_ORB):
        ...
        if event == "30738-10.htm":
            if check_ingredients(st, 1):
                ...
                if not st.getQuestItemsCount(MATILDS_ORB):
        ...
        #### WISH IV : I'd like to become the wisest person in the world.
            ...
            if wish == 1:
                if WISH_CHANCE <= 50:
                    autochat(
                        st.addSpawn(SUCCUBUS_OF_SEDUCTION, 200000),
                ...
                if WISH_CHANCE <= 33:
                    autochat(
                        st.addSpawn(GRIMA, 200000),
                        MESSAGES[GRIMA][st.getRandom(len(MESSAGES))],
                    )
                    autochat(
                        st.addSpawn(GRIMA, 200000),
                        MESSAGES[GRIMA][st.getRandom(len(MESSAGES))],
                ...
                if WISH_CHANCE <= 33:
                    ...
                    autochat(
                        st.addSpawn(WISDOM_CHEST, 120000),
                        "I contain the wisdom, I am the wisdom box!",
                    )
            npc.setBusy(False)
            return
    ...
    def onTalk(self, npc, player):
        ...
        cond = st.getInt("cond")
        id = st.getState()
        if npcId != ALCHEMIST_MATILD and id == CREATED:
            return htmltext
        if npcId == TORAI and st.getQuestItemsCount(FORBIDDEN_LOVE_SCROLL):
            ...
            if npc.isBusy():
                ...
                st.getQuestItemsCount(ALCHEMY_TEXT)
                and st.getQuestItemsCount(SECRET_BOOK)
            ):
                htmltext = "30738-05.htm"
            elif cond == 1 or (
                st.getQuestItemsCount(ALCHEMY_TEXT)
                and not st.getQuestItemsCount(SECRET_BOOK)
            ):
            ...
            if npcId == SUCCUBUS_OF_SEDUCTION:
                ...
                try:
                    ...
                    if st.getRandom(100) <= 50:
                        autochat(
                            npc, "It's time to come out my Remless... Bonaparterius!"
                        )
