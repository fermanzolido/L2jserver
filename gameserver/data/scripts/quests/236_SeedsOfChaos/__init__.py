...
NEEDED = {BLACK_ECHO_CRYSTAL: 1, SHINING_MEDALLION: 62}

SCROLL_ENCHANT_WEAPON_A = 729

# NPCs
KEKROPUS, WIZARD, KATENAR, ROCK, HARKILGAMED, MAO, RODENPICULA, NORNIL = (
    32138,
    31522,
    32235,
    32238,
...
class Quest(JQuest):
    ...
    def onEvent(self, event, st):
        if event == "1":  # Go talk to the wizard!
            ...
            event == "9"
        ):  # Go talk to Mao, no not the dictator Mao... the Vice Hierarch Mao.  <_<
            st.set("cond", "15")
            ...
            htmltext = (
                event + ".htm"
            )  # this is for having to go through pages upon pages of html text... <_<
        return htmltext
        ...
        if npcId == KEKROPUS:
            if id == CREATED:
                ...
                if player.getRace() != Race.Kamael:
                    ...
                    htmltext = (
                        "32138_02.htm"  # Successful start: Talk to me a few times,
                    )
            elif id == STARTED:
                if cond < 14:
                ...
                htmltext = (
                    "<html><body>You have already completed this quest.</body></html>"
                )
            ...
            npcId == MAO and id == STARTED
        ):  # Ok.  The deal with Mao is that he's supposed to port you to Mother Nornil, but since she's not yet in the spawnlist, he's just gonna tell ya where to find her.
