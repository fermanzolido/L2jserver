...
class Quest(JQuest):
    ...
    def onTalk(self, npc, player):
        ...
        if st:
            ...
            KAlliance = (
                st.getQuestItemsCount(Ketra_Alliance_One)
                + st.getQuestItemsCount(Ketra_Alliance_Two)
                + st.getQuestItemsCount(Ketra_Alliance_Three)
                + st.getQuestItemsCount(Ketra_Alliance_Four)
                + st.getQuestItemsCount(Ketra_Alliance_Five)
            )
        ...
        if st:
            if st.getState() == STARTED:
                ...
                if not partyMember.isAlliedWithKetra():
                    if (
                        (npcId in Ketra_One)
                        or (npcId in Ketra_Two)
                        or (npcId in Ketra_Three)
                    ):
                        item = 0
                        if cond <= 5:
