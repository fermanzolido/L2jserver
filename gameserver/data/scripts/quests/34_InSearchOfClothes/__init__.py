...
class Quest(JQuest):
    ...
    def onEvent(self, event, st):
        ...
        if event == "30088-5.htm" and cond == 6:
            if (
                st.getQuestItemsCount(SUEDE) >= 3000
                and st.getQuestItemsCount(THREAD) >= 5000
                and st.getQuestItemsCount(SPIDERSILK) == 1
            ):
    ...
    def onTalk(self, npc, player):
        ...
        if (
            npcId == 30088
            and cond == 0
            and st.getQuestItemsCount(MYSTERIOUS_CLOTH) == 0
        ):
