...
class Quest(JQuest):
    ...
    def onTalk(self, npc, player):
        ...
        if npcId == 30041 and st.getInt("cond") == 0 and st.getInt("onlyone") == 0:
            ...
            and (
                st.getQuestItemsCount(JACKSONS_RECEIPT_ID) != 0
                and st.getQuestItemsCount(SILVIAS_RECEIPT_ID) != 0
                and st.getQuestItemsCount(RANTS_RECEIPT_ID) != 0
            )
            == 0
        ):
        ...
        if id == STARTED:
            if (
                npcId == 30002
                and st.getInt("cond") != 0
                and st.getQuestItemsCount(HEAVY_WOOD_BOX_ID) != 0
            ):
                st.takeItems(
                    HEAVY_WOOD_BOX_ID, st.getQuestItemsCount(HEAVY_WOOD_BOX_ID)
                )
                ...
                npcId == 30002
                and st.getInt("cond") != 0
                and st.getQuestItemsCount(JACKSONS_RECEIPT_ID) != 0
            ):
                htmltext = "30002-02.htm"
            elif (
                npcId == 30003
                and st.getInt("cond") != 0
                and st.getQuestItemsCount(CLOTH_BUNDLE_ID) != 0
            ):
                ...
                if st.getQuestItemsCount(SILVIAS_RECEIPT_ID) == 0:
                    ...
                    st.giveItems(
                        LESSER_HEALING_POTION_ID, int(Config.RATE_QUESTS_REWARD)
                    )
                ...
                npcId == 30054
                and st.getInt("cond") != 0
                and st.getQuestItemsCount(CLAY_POT_ID) != 0
            ):
