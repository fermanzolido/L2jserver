    ...
    def onAdvEvent(self, event, npc, player):
        ...
        if event == "31328-02.htm":  # call 31328-03.htm
            ...
            ghost.broadcastPacket(
                CreatureSay(ghost.getObjectId(), 0, ghost.getName(), "Who awoke me?")
            )
    ...
    def onTalk(self, npc, player):
        ...
        if npcId == Innocentin:
            if state == CREATED:
                ...
                if not st2 == None:
                    if (
                        st2.getState().getName() == "Completed"
                        and player.getLevel() >= 64
                    ):
                        htmltext = (
                            "31328-01.htm"  # previous quest finished, call 31328-02.htm
                        )
                    else:
            ...
            if cond == 2:
                ...
                if st.getQuestItemsCount(SilverKey) != 0:
                    ...
                    st.getQuestItemsCount(LidiaHairPin) == 0
                    and st.getQuestItemsCount(LidiaDiary) != 0
                ):
