...
class Quest(JQuest):
    ...
    def onEvent(self, event, st):
        ...
        if event == "1":
            ...
            if (
                level >= 19
                and classId == 0x26
                and st.getQuestItemsCount(ORB_OF_ABYSS) == 0
            ):
                ...
                level >= 19
                and classId == 0x26
                and st.getQuestItemsCount(ORB_OF_ABYSS) == 1
            ):
    ...
    def onTalk(self, npc, player):
        ...
        if npcId == 30330 and st.getInt("cond") == 0:
            ...
            if st.getQuestItemsCount(SIDRAS_LETTER1) == 1:
                ...
                st.getQuestItemsCount(BLANK_SHEET1) > 0
                or st.getQuestItemsCount(BLOODY_RUNE1) == 1
            ):
                htmltext = "30330-08.htm"
            elif (
                st.getQuestItemsCount(ANDARIEL_BOOK) == 0
                and st.getQuestItemsCount(PRAYER_OF_ADON)
                + st.getQuestItemsCount(GARMIEL_BOOK)
                + st.getQuestItemsCount(PENITENTS_MARK)
                + st.getQuestItemsCount(ASHEN_BONES)
                > 0
            ):
                htmltext = "30330-09.htm"
            elif (
                st.getQuestItemsCount(ANDARIEL_BOOK) == 1
                and st.getQuestItemsCount(GARMIEL_BOOK) == 1
            ):
            ...
            if st.getQuestItemsCount(SIDRAS_LETTER1) == 1:
                ...
                st.getQuestItemsCount(BLANK_SHEET1) == 5
                and st.getQuestItemsCount(BLOODY_RUNE1) == 0
            ):
                htmltext = "30377-03.htm"
            elif (
                st.getQuestItemsCount(BLOODY_RUNE1) > 0
                and st.getQuestItemsCount(BLOODY_RUNE1) < 5
            ):
                ...
                st.getQuestItemsCount(PRAYER_OF_ADON)
                + st.getQuestItemsCount(PENITENTS_MARK)
                + st.getQuestItemsCount(ASHEN_BONES)
                > 0
            ):
            ...
            if st.getQuestItemsCount(PRAYER_OF_ADON) == 1:
                ...
                st.getQuestItemsCount(PENITENTS_MARK) == 1
                and st.getQuestItemsCount(ASHEN_BONES) == 0
                and st.getQuestItemsCount(ANDARIEL_BOOK) == 0
            ):
                htmltext = "30375-05.htm"
            elif (
                st.getQuestItemsCount(PENITENTS_MARK) == 1
                and st.getQuestItemsCount(ASHEN_BONES) < 10
                and st.getQuestItemsCount(ASHEN_BONES) > 0
            ):
    ...
    def onKill(self, npc, player, isPet):
        ...
        if npcId == 20776:
            ...
            if (
                st.getInt("cond")
                and st.getQuestItemsCount(PENITENTS_MARK) == 1
                and st.getQuestItemsCount(ASHEN_BONES) < 10
            ):
