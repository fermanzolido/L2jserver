...
# QUEST DROPS
BLACK_BONE_NECKLACE, RED_BONE_NECKLACE, INCENSE_POUCH, GEM_OF_MAILLE = list(
    range(7178, 7182)
)

NECKLACE = {
    M_LIZARDMAN_GUARD: [RED_BONE_NECKLACE, 100, BLACK_BONE_NECKLACE, "3"],
    M_LIZARDMAN: [BLACK_BONE_NECKLACE, 100, RED_BONE_NECKLACE, "3"],
    M_LIZARDMAN_SCOUT: [BLACK_BONE_NECKLACE, 100, RED_BONE_NECKLACE, "3"],
}
DROPLIST = {
    ARANEID: [GEM_OF_MAILLE, 30, INCENSE_POUCH, "5"],
    ...
    def onEvent(self, event, st):
        ...
        if st.getState() != COMPLETED:
            if event == "30334-1.htm" and cond == 0:
                ...
                if (
                    st.getQuestItemsCount(BLACK_BONE_NECKLACE)
                    == st.getQuestItemsCount(RED_BONE_NECKLACE)
                    == 100
                    and cond == 3
                ):
                ...
                if (
                    st.getQuestItemsCount(INCENSE_POUCH)
                    == st.getQuestItemsCount(GEM_OF_MAILLE)
                    == 30
                    and cond == 5
                ):
