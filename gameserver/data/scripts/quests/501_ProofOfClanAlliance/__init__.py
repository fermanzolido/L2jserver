...
class Quest(JQuest):
    ...
    def onAdvEvent(self, event, npc, player):
        ...
        if player.isClanLeader():
            if event == "30756-07.htm":
                ...
                npc.broadcastPacket(
                    MagicSkillUser(
                        npc, player, skill.getDisplayId(), 1, skill.getHitTime(), 0
                    )
                )
    ...
    def onTalk(self, npc, player):
        ...
        if npcId == SIR_KRISTOF_RODEMAI:
            ...
            if (
                id == CREATED
            ):  # not a valid leader (has not started the quest yet) or a clan member
                leaderst = leader(player)
                if leaderst:
                ...
                if part == 1:  # and not symbol :
                    ...
                    part == 5
                    and st.getQuestItemsCount(HERB_OF_HARIT)
                    and st.getQuestItemsCount(HERB_OF_VANOR)
                    and st.getQuestItemsCount(HERB_OF_OEL_MAHUM)
                    and st.getQuestItemsCount(BLOOD_OF_EVA)
                    and isEffected(player, 4082)
                ):
                    htmltext = "08"
                    st.giveItems(VOUCHER_OF_FAITH, 1)
                    st.giveItems(POTION_OF_RECOVERY, 1)
                    for item in list(range(3832, 3836)) + [ANTIDOTE_RECIPE]:
                        st.takeItems(item, -1)
                    st.set("part", "6")
                    st.set("cond", "4")
            ...
            if leaderst:
                ...
                if id == STARTED:
                    ...
                    if (
                        part == 3
                        and leaderst.getQuestItemsCount(ANTIDOTE_RECIPE)
    ...
    def onKill(self, npc, player, isPet):
        ...
        if npcId in CHESTS and part == 4:
            ...
            if (self.chests - wins) == 12 or (wins < 4 and not leaderst.getRandom(4)):
                ...
                npc.broadcastPacket(
                    CreatureSay(
                        npc.getObjectId(), 0, npc.getName(), "###### BINGO! ######"
                    )
                )
            self.chests += 1
        elif npcId in list(MOBS.keys()):
            st = player.getQuestState(qn)
            if not st:
...
for i in list(MOBS.keys()) + CHESTS:
    QUEST.addKillId(i)
