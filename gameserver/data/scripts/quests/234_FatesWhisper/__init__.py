...
DROPLIST = {29020: [7, RED_PIPETTE_KNIFE, 10]}  # Baium...NOTE: rate may need adjustment
...
class Quest(JQuest):
        ...
        ## FINAL ITEM EXCHANGE SECTION
            ...
            for Item in st.getPlayer().getInventory().getItems():
                ...
                if (
                    ...
                    if (
                        pDam >= TopBGradeWeaponData[weaponType][0]
                        or mDam >= TopBGradeWeaponData[weaponType][1]
                    ):
                        htmltext += (
                            '<a action="bypass -h Quest 234_FatesWhisper selectBGrade_'
            ...
            htmltext = (
                "<html><body>Maestro Reorin:<br>Please choose which weapon you wish to give me to melt, from the below list:<br>"
                + htmltext
                + "</body></html>"
            )
            ...
            if Item and not Item.isEquipped():
                st.getPlayer().destroyItem(
                    "234_FatesWhisper", bGradeObjId, 1, st.getPlayer(), 0
                )
                st.set("bypass", "1")
    ...
    def onTalk(self, npc, player):
        ...
        # if quest is accepted and in progress
            ...
            cond = st.getInt("cond")
            if npcId == NPC[0]:
                if cond == 1 and not st.getQuestItemsCount(
                    REIRIAS_SOUL_ORB
                ):  # waiting for the orb
                    htmltext = "31002-04b.htm"
                elif (
                    cond == 1
                ):  # got the orb!  Go to the next step (infernium scepter pieces)
                    st.takeItems(REIRIAS_SOUL_ORB, 1)
                    htmltext = "31002-05.htm"
                    st.set("cond", "2")
                # waiting for infernium scepter pieces
                elif cond == 2 and (
                    st.getQuestItemsCount(KERMONS_INFERNIUM_SCEPTER)
                    + st.getQuestItemsCount(GOLCONDAS_INFERNIUM_SCEPTER)
                    + st.getQuestItemsCount(HALLATES_INFERNIUM_SCEPTER)
                    < 3
                ):
                    htmltext = "31002-05c.htm"
                elif (
                    cond == 2
                ):  # got the infernium scepter pieces!  Go to the next step (infernium Varnish)
                ...
                # waiting for infernium varnish
                    ...
                    cond == 3
                ):  # got the infernium varnish!  Go to the next step (Leorin's Hammer)
                ...
                # waiting for Leorin's Hammer
                    ...
                    cond == 4
                ):  # got Leorin's Hammer!  Go to the next step (Leorin's Mold)
            ...
            # came to take the varnish
                ...
                npcId == NPC[1]
                and cond == 3
                and not st.getQuestItemsCount(INFERNIUM_VARNISH)
            ):
                htmltext = "30182-01.htm"
            # you already got the varnish...why are you back?
            elif npcId == NPC[1] and (
                cond >= 3 or st.getQuestItemsCount(INFERNIUM_VARNISH)
            ):
            ...
            # go to take the mold
                ...
                npcId == NPC[2]
                and cond == 4
                and not st.getQuestItemsCount(REORINS_HAMMER)
            ):
            ...
            ## KASPAR
                ...
                if (
                    cond == 7
                    and st.getQuestItemsCount(PIPETTE_KNIFE)
                    and not st.getQuestItemsCount(RED_PIPETTE_KNIFE)
                ):
                ...
                if (
                    cond == 7
                    and not st.getQuestItemsCount(PIPETTE_KNIFE)
                    and st.getQuestItemsCount(RED_PIPETTE_KNIFE)
                ):
            ...
            ## CHESTS FROM RAIDBOSSES
                ...
                if (
                    npcId == NPC[5]
                    and st.getQuestItemsCount(KERMONS_INFERNIUM_SCEPTER) == 0
                ):
                ...
                # Golkonda's Chest
                    ...
                    npcId == NPC[6]
                    and st.getQuestItemsCount(GOLCONDAS_INFERNIUM_SCEPTER) == 0
                ):
                ...
                # Hallate's Chest
                    ...
                    npcId == NPC[7]
                    and st.getQuestItemsCount(HALLATES_INFERNIUM_SCEPTER) == 0
                ):
    ...
    def onAttack(self, npc, player, damage, isPet):
        ...
        if value == st.getInt("cond") and npcId == 29020:
            if (
                ...
                and player.getActiveWeaponItem().getItemId() == PIPETTE_KNIFE
                and st.getRandom(100) < chance
                and st.getQuestItemsCount(dropId) == 0
            ):
