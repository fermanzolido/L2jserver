...
class Quest(JQuest):
    ...
    def onEvent(self, event, st):
        ...
        if event == "30223-04.htm":
            if (
                st.getQuestItemsCount(ARUJIENS_LETTER1) == 0
                and st.getQuestItemsCount(ARUJIENS_LETTER2) == 0
                and st.getQuestItemsCount(ARUJIENS_LETTER3) == 0
            ):
        ...
        if npcId == ARUJIEN and id == CREATED:
            ...
            if npcId == MIRABEL and cond == 1:
                if st.getQuestItemsCount(ARUJIENS_LETTER1):
                    ...
                    st.getQuestItemsCount(ARUJIENS_LETTER2)
                    or st.getQuestItemsCount(ARUJIENS_LETTER3)
                    or st.getQuestItemsCount(POETRY_BOOK)
                    or st.getQuestItemsCount(GREENIS_LETTER)
                ):
                    htmltext = "30146-02.htm"
            elif (
                npcId == HERBIEL
                and cond == 2
                and st.getQuestItemsCount(ARUJIENS_LETTER1) == 0
            ):
                if st.getQuestItemsCount(ARUJIENS_LETTER2):
                    ...
                    st.getQuestItemsCount(ARUJIENS_LETTER3)
                    or st.getQuestItemsCount(POETRY_BOOK)
                    or st.getQuestItemsCount(GREENIS_LETTER)
                ):
