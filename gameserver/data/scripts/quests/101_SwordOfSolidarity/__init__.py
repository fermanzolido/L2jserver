    ...
    def onTalk(self, npc, player):
        ...
        if id == STARTED:
            if (
                ...
                and (st.getQuestItemsCount(ROIENS_LETTER_ID) == 1)
            ):
                htmltext = "30008-05.htm"
            elif (
                npcId == 30008
                and st.getInt("cond") >= 2
                and st.getQuestItemsCount(ROIENS_LETTER_ID) == 0
                and st.getQuestItemsCount(ALLTRANS_NOTE_ID) == 0
            ):
                if st.getQuestItemsCount(BROKEN_BLADE_TOP_ID) and st.getQuestItemsCount(
                    BROKEN_BLADE_BOTTOM_ID
                ):
                    htmltext = "30008-12.htm"
                if (
                    st.getQuestItemsCount(BROKEN_BLADE_TOP_ID)
                    + st.getQuestItemsCount(BROKEN_BLADE_BOTTOM_ID)
                ) <= 1:
                ...
                and st.getQuestItemsCount(ROIENS_LETTER_ID) == 0
                and st.getQuestItemsCount(ALLTRANS_NOTE_ID)
            ):
                ...
                and st.getInt("cond") >= 2
                and st.getQuestItemsCount(ROIENS_LETTER_ID) == 0
                and st.getQuestItemsCount(HOWTOGO_RUINS_ID) > 0
            ):
                if (
                    st.getQuestItemsCount(BROKEN_BLADE_TOP_ID)
                    + st.getQuestItemsCount(BROKEN_BLADE_BOTTOM_ID)
                ...
                if st.getQuestItemsCount(BROKEN_BLADE_TOP_ID) and st.getQuestItemsCount(
                    ...
                    st.set("cond", "4")
                    st.takeItems(
                        HOWTOGO_RUINS_ID, st.getQuestItemsCount(HOWTOGO_RUINS_ID)
                    )
                    st.takeItems(
                        BROKEN_BLADE_TOP_ID, st.getQuestItemsCount(BROKEN_BLADE_TOP_ID)
                    )
                    st.takeItems(
                        BROKEN_BLADE_BOTTOM_ID,
                        st.getQuestItemsCount(BROKEN_BLADE_BOTTOM_ID),
                ...
                and st.getQuestItemsCount(BROKEN_SWORD_HANDLE_ID)
            ):
