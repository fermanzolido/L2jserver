...
class Quest(JQuest):
    ...
    def onTalk(self, npc, player):
        ...
        if st:
            ...
            VAlliance = (
                st.getQuestItemsCount(Varka_Alliance_One)
                + st.getQuestItemsCount(Varka_Alliance_Two)
                + st.getQuestItemsCount(Varka_Alliance_Three)
                + st.getQuestItemsCount(Varka_Alliance_Four)
                + st.getQuestItemsCount(Varka_Alliance_Five)
            )
        ...
        if st:
            if st.getState() == STARTED:
                ...
                if not partyMember.isAlliedWithVarka():
                    if (
                        (npcId in Varka_One)
                        or (npcId in Varka_Two)
                        or (npcId in Varka_Three)
                    ):
                        item = 0
                        if cond <= 5:
