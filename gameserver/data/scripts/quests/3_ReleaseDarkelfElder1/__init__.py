...
# ITEMS
ONYX_BEAST_EYE, TAINT_STONE, SUCCUBUS_BLOOD = list(range(1081, 1084))
...
class Quest(JQuest):
    ...
    def onKill(self, npc, player, isPet):
        ...
        if st.getInt("cond") == 1:
            ...
            elif npcId in [TAINTED_ZOMBIE, STINK_ZOMBIE] and not st.getQuestItemsCount(
                TAINT_STONE
            ):
            ...
            ] and not st.getQuestItemsCount(SUCCUBUS_BLOOD):
                st.giveItems(SUCCUBUS_BLOOD, 1)
            if (
                st.getQuestItemsCount(ONYX_BEAST_EYE)
