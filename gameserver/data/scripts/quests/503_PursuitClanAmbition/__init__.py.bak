...
DROPLIST = {
    ...
    20654: [
        ...
        10,
        25,
        [Broke_Power_Stone, Power_Stones, Nebulite_Crystals],
    ],  # Giant Soldier
    20656: [
        5,
        10,
...
def offlineMemberExit(st):
    ...
    con = L2DatabaseFactory.getInstance().getConnection(false)
    offline = con.prepareStatement(
        "DELETE FROM character_quests WHERE name = ? and char_id IN (SELECT obj_id FROM characters WHERE clanId =? AND online=0"
    )
...
def exit503(completed, st):
    ...
    try:
        ...
        for i in members:
            st.getPlayer().getClan().getClanMember(i).getPlayerInstance().getQuestState(
                qn
            ).exitQuest(1)
...
class Quest(JQuest):
    ...
    def onEvent(self, event, st):
        ...
        # Events Cleo
            ...
            spawnedNpc.broadcastPacket(
                CreatureSay(
                    spawnedNpc.getObjectId(), 0, spawnedNpc.getName(), "War and Death"
                )
            )
    ...
    def onTalk(self, npc, player):
        ...
        if id == CREATED and npcId == Gustaf:
            ...
            if player.getClan():  # has Clan
                if isLeader:  # check if player is clan leader
                    ...
                    if st.getQuestItemsCount(
                        ...
                        clanLevel == 4
                    ):  # if clanLevel == 4 you can take this quest, because repeatable
            ...
            player.getClan() and player.getClan().getLevel() >= 5
            ...
            if isLeader:
                ...
                if npcId == Gustaf:
                    ...
                    if cond == 4:
                        ...
                        if (
                            st.getQuestItemsCount(Power_Stones) > 9
                            and st.getQuestItemsCount(Nebulite_Crystals) > 9
                        ):
    ...
    def onKill(self, npc, player, isPet):
        ...
        if leader_st:
            if leader_st.getState() != PROGRESS:
                return
            npcId = npc.getNpcId()
            condition, maxcount, chance, itemList = DROPLIST[npcId]
