...
# Quest info
QUEST_NUMBER, QUEST_NAME, QUEST_DESCRIPTION = (
    374,
    "WhisperOfDreams1",
...
SHOP_LIST = {
    5485: [
        "etc_leather_yellow_i00",
        4,
        10450,
        "Sealed Tallum Tunic Textures",
    ],  # 4xTallum Tunic Textures: 10450a
...
def render_shop():
    ...
    for i in list(SHOP_LIST.keys()):
        html += (
            ...
            + SHOP_LIST[i][0]
            + " width=32 height=32 align=left></td><td width=365 valign=top><table border=0 width=100%>"
        )
        html += (
            '<tr><td><a action="bypass -h Quest 374_WhisperOfDreams1 '
            + str(i)
            + '"><font color="FFFFFF">'
            + SHOP_LIST[i][3]
            + " x"
        ...
        html += (
            ...
            + str(i)
            + '"><font color="B09878">'
            + str(SHOP_LIST[i][2] * ADENA_X)
            + " adena</font></a></td></tr></table></td></tr>"
        )
...
class Quest(JQuest):
    ...
    def onTalk(self, npc, player):
        ...
        npcId = npc.getNpcId()
        id = st.getState()
        if npcId != MANAKIA and id != STARTED:
            return htmltext
    ...
    def onKill(self, npc, player, isPet):
        ...
        if partyMember:
            ...
            if st.getRandom(DROP_MAX_2) < DROP_RATE_2 and not st.getQuestItemsCount(
                SEALD_MSTONE
            ):
