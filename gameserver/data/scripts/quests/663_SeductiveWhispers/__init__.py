...
class Quest(JQuest):
    ...
    def onEvent(self, event, st):
        ...
        if event == "Wilbert_IWantToPlay.htm":  # quest accepted
            ...
            beads = st.getQuestItemsCount(
                SPIRIT_BEAD
            )  # get beads count, it must be > 1
            if beads < 1:
                htmltext = "Wilbert_Practice_NotEnoughBeads.htm"
            else:
                ...
                if (
                    random < WIN_ROUND_CHANCE
                ):  # random value is in range [0..WIN_ROUND_CHANCE]
            ...
            beads = st.getQuestItemsCount(SPIRIT_BEAD)
            if (
                beads < 50 and round == 0
            ):  # check for 50 beads when game just starts only (round=0)
                htmltext = "Wilbert_Practice_NotEnoughBeads.htm"
            else:
                ...
                if (
                    random > WIN_ROUND_CHANCE
                ):  # random value is in range [WIN_ROUND_CHANCE..100]
                    htmltext = "Wilbert_PlayLose.htm"
                    st.set("round", "0")  # restart game
                else:  # next round won
                    round = round + 1
                    htmltext = st.showHtmlFile("Wilbert_PlayWin.htm").replace(
                        "NROUND", str(round)
                    )
                    if round == 1:
                    ...
                    if round == 3:
                        htmltext = htmltext.replace(
                            "MYPRIZE", "110,000 adena, D-grade Enchant Weapon Scroll"
                        )
                    if round == 4:
                        htmltext = htmltext.replace(
                            "MYPRIZE", "199,000 adena, C-grade Enchant Weapon Scroll"
                        )
                    if round == 5:
                        htmltext = htmltext.replace(
                            "MYPRIZE", "388,000 adena, 1 recipe for a B-grade weapon"
                        )
                    if round == 6:
                        htmltext = htmltext.replace(
                            "MYPRIZE",
            ...
            event == "Wilbert_TakePrize.htm"
        ):  # player won round and wants to stop game and take prize
            round = st.getInt("round")
            if round == 0:  # player did not win any round but wants to take prize? O_o
                htmltext = (
                    "<html><body>You did not win any round! No prizes.</body></html>"
                )
                return htmltext
