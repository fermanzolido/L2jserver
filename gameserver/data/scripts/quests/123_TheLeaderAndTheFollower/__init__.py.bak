...
NEWYEAR = 31961
# MOBS and DROPS
DROPLIST = {27321: [BLOOD, 60, 10, 1, 0], 27322: [LEG, 70, 8, 7, 1]}
...
class Quest(JQuest):
    ...
    def onEvent(self, event, st):
        ...
        if event == "31961-02.htm":
            ...
            cm_apprentice = (
                st.getPlayer().getClan().getClanMember(st.getPlayer().getApprentice())
            )
                    ...
                    if apprentice:
                        ...
                        if ap_quest:
                            ...
                            if ap_cond == 3:
                                ...
                                if DEBUG:
                                    htmltext = "31961-FF.htm"
                                st.exitQuest(1)
                        else:
                            if DEBUG:
                                htmltext = "31961-FE.htm"
                            st.exitQuest(1)
                    else:
                        if DEBUG:
                            htmltext = "31961-FD.htm"
                        st.exitQuest(1)
                else:
                    if DEBUG:
                        htmltext = "31961-FC.htm"
                    st.exitQuest(1)
            else:
                if DEBUG:
                    htmltext = "31961-FB.htm"
                st.exitQuest(1)
        else:
            if DEBUG:
                htmltext = "31961-FA.htm"
            st.exitQuest(1)
        return htmltext
