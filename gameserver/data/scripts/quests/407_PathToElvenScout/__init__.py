...
class Quest(JQuest):
    ...
    def onTalk(self, npc, player):
        ...
        if npcId == 30328 and st.getInt("cond") == 0:
            ...
            npcId == 30328
            and st.getInt("cond")
            and st.getQuestItemsCount(REORIA_LETTER2) > 0
        ):
            ...
            and st.getInt("cond")
            and st.getQuestItemsCount(REORIA_LETTER2) == 0
            and st.getQuestItemsCount(HONORARY_GUARD) == 0
        ):
            ...
            and st.getInt("cond")
            and st.getQuestItemsCount(REORIA_LETTER2) > 0
            and st.getQuestItemsCount(PRIGUNS_TEAR_LETTER1)
            + st.getQuestItemsCount(PRIGUNS_TEAR_LETTER2)
            + st.getQuestItemsCount(PRIGUNS_TEAR_LETTER3)
            + st.getQuestItemsCount(PRIGUNS_TEAR_LETTER4)
            == 0
        ):
            htmltext = "30337-01.htm"
        elif (
            npcId == 30337
            and st.getQuestItemsCount(MORETTIS_LETTER) < 1
            and st.getQuestItemsCount(PRIGUNS_LETTER) == 0
            and st.getQuestItemsCount(HONORARY_GUARD) == 0
        ):
            if (
                st.getQuestItemsCount(PRIGUNS_TEAR_LETTER1)
                + st.getQuestItemsCount(PRIGUNS_TEAR_LETTER2)
                + st.getQuestItemsCount(PRIGUNS_TEAR_LETTER3)
                + st.getQuestItemsCount(PRIGUNS_TEAR_LETTER4)
                < 1
            ...
            npcId == 30426
            and st.getInt("cond")
            and st.getQuestItemsCount(MORETTIS_LETTER)
            and st.getQuestItemsCount(MORETTIS_HERB)
        ):
            ...
            npcId == 30426
            and st.getInt("cond")
            and st.getQuestItemsCount(PRIGUNS_LETTER)
        ):
    ...
    def onKill(self, npc, player, isPet):
        ...
        if npcId == 20053:
            ...
            if (
                st.getInt("cond")
                and st.getQuestItemsCount(MORETTIS_HERB) == 1
                and st.getQuestItemsCount(MORETTIS_LETTER) == 1
                and st.getQuestItemsCount(RUSTED_KEY) == 0
                and st.getRandom(10) < 6
            ):
