...
class Quest(JQuest):
    ...
    def onEvent(self, event, st):
        ...
        if event == "30191-04.htm":
            ...
            ):  # Condition 12 meet the Lord Kakai (Orc Master)
    ...
    def onTalk(self, npc, player):
        ...
        if id == CREATED:  # Check if is starting the quest
            ...
            if (
                cond == 1
            ):  # Condition 1 take the letter to Hierarch Asterios (Elven Master)
                if npcId == 30191:
                ...
                cond == 3
            ):  # Condition 3 bring back the Breath of winds and Seed of Verdure to Asterios
                if (
                    npcId == 30154
                    and st.getQuestItemsCount(BREATH_OF_WINDS_ID)
                    and st.getQuestItemsCount(SEED_OF_VERDURE_ID)
                ):
                ...
                cond == 4
            ):  # Condition 4 take the letter to Tetrarch Thifiell (Dark Elven Master)
                if npcId == 30154:
                ...
                cond == 6
            ):  # Condition 6 get 10 of each, Stakato ichor, honey dew and basilisk plasma
                if npcId == 30464 and st.getQuestItemsCount(ORDER_OF_CLAYTON_ID):
                    htmltext = "30464-02.htm"
            elif (
                cond == 7
            ):  # Condition 7 bring back the Stakato ichor, honey dew and basilisk plasma to Magister Clayton
                if (
                    npcId == 30464
                    and st.getQuestItemsCount(ORDER_OF_CLAYTON_ID)
                    and st.getQuestItemsCount(STAKATO_ICHOR_ID)
                    and st.getQuestItemsCount(HONEY_DEW_ID)
                    and st.getQuestItemsCount(BASILISK_PLASMA_ID)
                ):
                    htmltext = "30464-03.htm"
                    st.set("cond", "8")
            elif (
                cond == 8
            ):  # Condition 8 take the Stakato ichor, honey dew and basilisk plasma to Thifiell
                if (
                    npcId == 30358
                    and st.getQuestItemsCount(ORDER_OF_CLAYTON_ID)
                    and st.getQuestItemsCount(STAKATO_ICHOR_ID)
                ...
                cond == 9
            ):  # Condition 9 take the Elven and Dark Elven scroll to Hollint
                if (
                    npcId == 30191
                    and st.getQuestItemsCount(SCROLL_OF_ELF_TRUST_ID)
                    and st.getQuestItemsCount(SCROLL_OF_DARKELF_TRUST_ID)
                ):
                ...
                10,
                11,
            ]:  # Condition 10 meet the Seresin or Condition 11 A lack of Experience
                if npcId == 30191:
                ...
                cond == 15
            ):  # Condition 15 bring back the Parasite of lota to Seer Manakia
                ...
                cond == 16
            ):  # Condition 16 bring the letter of Manakia to the Lord Kakai
                ...
                if (
                    npcId == 30621
                    and st.getQuestItemsCount(ORDER_OF_NICHOLA_ID)
                    and st.getQuestItemsCount(HEART_OF_PORTA_ID) == 10
                ):
                ...
                cond == 22
            ):  # Condition 22 take the Orc and Dwarven scroll to High Priest Hollint
                if (
                    npcId == 30191
                    and st.getQuestItemsCount(SCROLL_OF_DWARF_TRUST_ID)
                    and st.getQuestItemsCount(SCROLL_OF_ORC_TRUST_ID)
                ):
                ...
                cond == 23
            ):  # Condition 23 take the Recommendation of Hollin to the High Priest Biotin
                if npcId == 30191:
                    htmltext = "30191-07.htm"
                elif npcId == 30031 and st.getQuestItemsCount(
                    RECOMMENDATION_OF_HOLLIN_ID
                ):
    ...
    def onKill(self, npc, player, isPet):
        ...
        npcId = npc.getNpcId()
        cond = st.getInt("cond")
        if cond == 2 and npcId in [
            20013,
            20019,
            20036,
            20044,
        ]:  # Condition 2 kill the Luel of Zephy and Aktea of the Woods
            if (
                npcId in [20036, 20044]
                and st.getQuestItemsCount(BREATH_OF_WINDS_ID) == 0
            ):
                st.set("id", str(st.getInt("id") + 1))
                if st.getRandom(100) < (st.getInt("id") * 33):
                    st.playSound("Itemsound.quest_before_battle")
                    st.addSpawn(
                        27120, 9410, 50301, -3713, 600000
                    )  ### FIXME ### Temp fix for spawn
            ...
            #         st.addSpawn(27120)                   # The original spawn code
                ...
                npcId in [20013, 20019]
                and st.getQuestItemsCount(SEED_OF_VERDURE_ID) == 0
            ):
                st.set("id", str(st.getInt("id") + 1))
                if st.getRandom(100) < (st.getInt("id") * 33):
                    st.playSound("Itemsound.quest_before_battle")
                    st.addSpawn(
                        27121, 16895, 47210, -3673, 600000
                    )  ### FIXME ### Temp fix for spawn
                    st.addRadar(16895, 47210, -3673)
                    return (
                        "Actea Of Verdant Wilds has spawned at X=16895 Y=47210 Z=-3673"
                    )
            ...
            if st.getQuestItemsCount(required) and count < maxqty:
                ...
                if count + 1 == maxqty:  # Check if got enough number of items
                    ...
                    if npcId in [
                        ...
                        if (
                            st.getQuestItemsCount(BASILISK_PLASMA_ID)
                            and st.getQuestItemsCount(HONEY_DEW_ID)
                            and st.getQuestItemsCount(STAKATO_ICHOR_ID)
                        ):
                            st.set("cond", "7")
