        ...
        if npcId == 30568 and st.getInt("cond") == 0 and st.getInt("onlyone") == 0:
            ...
            and (
                (
                    ...
                    + st.getQuestItemsCount(LETTER_TO_DARKELF_ID)
                )
                == 0
            )
        ):
    ...
    def onKill(self, npc, player, isPet):
        ...
        if npcId == 27041:
            ...
            if st.getInt("cond") == 1:
                if (
                    st.getQuestItemsCount(HATOSS_ORDER1_ID)
                    and st.getQuestItemsCount(LETTER_TO_HUMAN_ID) == 0
                ):
                    st.giveItems(LETTER_TO_HUMAN_ID, 1)
                    st.playSound("ItemSound.quest_itemget")
                if (
                    st.getQuestItemsCount(HATOSS_ORDER2_ID)
                    and st.getQuestItemsCount(LETTER_TO_DARKELF_ID) == 0
                ):
                    st.giveItems(LETTER_TO_DARKELF_ID, 1)
                    st.playSound("ItemSound.quest_itemget")
                if (
                    st.getQuestItemsCount(HATOSS_ORDER3_ID)
                    and st.getQuestItemsCount(LETTER_TO_ELF_ID) == 0
                ):
