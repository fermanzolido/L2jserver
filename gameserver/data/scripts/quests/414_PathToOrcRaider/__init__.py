...
class Quest(JQuest):
    ...
    def onTalk(self, npc, player):
        ...
        npcId = npc.getNpcId()
        id = st.getState()
        if npcId != KARUKIA and id != STARTED:
            return htmltext
        ...
        if npcId == KARUKIA and cond == 0:
            if (
                playerLvl >= 19
                and playerClassID == 0x2C
                and st.getQuestItemsCount(MARK_OF_RAIDER) == 0
                and st.getQuestItemsCount(GOBLIN_DWELLING_MAP) == 0
            ):
                ...
                playerLvl >= 19
                and playerClassID == 0x2C
                and st.getQuestItemsCount(MARK_OF_RAIDER) == 1
            ):
            ...
            npcId == KARUKIA
            and cond
            and st.getQuestItemsCount(GOBLIN_DWELLING_MAP) == 1
            and st.getQuestItemsCount(KURUKA_RATMAN_TOOTH) < 10
        ):
            ...
            and st.getQuestItemsCount(GOBLIN_DWELLING_MAP) == 1
            and st.getQuestItemsCount(KURUKA_RATMAN_TOOTH) >= 10
            and st.getQuestItemsCount(BETRAYER_UMBAR_REPORT) == 0
        ):
            ...
            npcId == KARUKIA
            and cond
            and st.getQuestItemsCount(BETRAYER_UMBAR_REPORT)
            ...
            and cond
            and st.getQuestItemsCount(BETRAYER_UMBAR_REPORT)
            and st.getQuestItemsCount(HEAD_OF_BETRAYER) == 0
        ):
    ...
    def onKill(self, npc, player, isPet):
        ...
        if npcId == GOBLIN_TOMB_RAIDER_LEADER:
            if (
                ...
                and st.getQuestItemsCount(GOBLIN_DWELLING_MAP) == 1
                and st.getQuestItemsCount(KURUKA_RATMAN_TOOTH) < 10
                and st.getQuestItemsCount(GREEN_BLOOD) < 40
            ):
                if st.getQuestItemsCount(GREEN_BLOOD) > 20:
                    if st.getRandom(100) < (
                        (st.getQuestItemsCount(GREEN_BLOOD) - 20) * 5
                    ):
