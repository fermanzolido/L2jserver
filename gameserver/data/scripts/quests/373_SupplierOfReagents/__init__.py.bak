...
def render_urn(st, page):
    stone, ingredient, catalyst = (
        st.getInt("mixing"),
        st.getInt("ingredient"),
        st.getInt("catalyst"),
    )
    if page == "Start":
        ...
        for i in MATS:
            if st.getQuestItemsCount(i):
                ...
                html += (
                    ...
                    + ITEMS[i][1]
                    + '</td><td><button value=X1 action="bypass -h Quest 373_SupplierOfReagents x_1_'
                    + page[1]
                    + "_"
                    + str(i)
                    + '" width=40 height=15 fore=sek.cbui92><button value=X10 action="bypass -h Quest 373_SupplierOfReagents x_2_'
                    + page[1]
                    + "_"
                    + str(i)
