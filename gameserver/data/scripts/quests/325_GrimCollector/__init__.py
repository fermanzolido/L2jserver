...
def pieces(st):
    return (
        st.getQuestItemsCount(ZOMBIE_HEAD1_ID)
...
def payback(st):
    ...
    if count:
        amount = (
            30 * st.getQuestItemsCount(ZOMBIE_HEAD1_ID)
            + 20 * st.getQuestItemsCount(ZOMBIE_HEART1_ID)
            + 20 * st.getQuestItemsCount(ZOMBIE_LIVER1_ID)
            + 100 * st.getQuestItemsCount(SKULL1_ID)
            + 40 * st.getQuestItemsCount(RIB_BONE1_ID)
            + 14 * st.getQuestItemsCount(SPINE1_ID)
            + 14 * st.getQuestItemsCount(ARM_BONE1_ID)
            + 14 * st.getQuestItemsCount(THIGH_BONE1_ID)
            + 341 * st.getQuestItemsCount(COMPLETE_SKELETON_ID)
        )
        if count > 10:
            amount += 1629
...
class Quest(JQuest):
    ...
    def onEvent(self, event, st):
        ...
        if event == "30336-03.htm":
            ...
            st.giveItems(
                ADENA_ID, 543 + (341 * st.getQuestItemsCount(COMPLETE_SKELETON_ID))
            )
            st.takeItems(COMPLETE_SKELETON_ID, -1)
        elif event == "30342-03.htm":
            if (
                st.getQuestItemsCount(SPINE1_ID)
                and st.getQuestItemsCount(ARM_BONE1_ID)
                and st.getQuestItemsCount(SKULL1_ID)
                and st.getQuestItemsCount(RIB_BONE1_ID)
                and st.getQuestItemsCount(THIGH_BONE1_ID)
            ):
                st.takeItems(SPINE1_ID, 1)
                st.takeItems(SKULL1_ID, 1)
                st.takeItems(ARM_BONE1_ID, 1)
    ...
    def onTalk(self, npc, player):
        ...
        if npcId == 30336 and cond == 0:
            ...
            npcId == 30434
            and cond
            and st.getQuestItemsCount(ANATOMY_DIAGRAM_ID)
            and not pieces(st)
        ):
            ...
            and st.getQuestItemsCount(ANATOMY_DIAGRAM_ID)
            and pieces(st)
            and not st.getQuestItemsCount(COMPLETE_SKELETON_ID)
        ):
            ...
            and st.getQuestItemsCount(ANATOMY_DIAGRAM_ID)
            and pieces(st)
            and st.getQuestItemsCount(COMPLETE_SKELETON_ID)
        ):
