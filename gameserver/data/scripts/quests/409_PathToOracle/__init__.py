...
class Quest(JQuest):
    ...
    def onEvent(self, event, st):
        ...
        if event == "1":
            ...
            if (
                level >= 19
                and classId == 0x19
                and st.getQuestItemsCount(LEAF_OF_ORACLE) == 0
            ):
                st.set("cond", "1")
                st.setState(STARTED)
                ...
                level >= 19
                and classId == 0x19
                and st.getQuestItemsCount(LEAF_OF_ORACLE) == 1
            ):
    ...
    def onTalk(self, npc, player):
        ...
        if npcId == 30293 and st.getInt("cond") == 0:
            ...
            npcId == 30293
            and st.getInt("cond")
            and st.getQuestItemsCount(CRYSTAL_MEDALLION)
        ):
            if (
                st.getQuestItemsCount(MONEY_OF_SWINDLER) == 0
                and st.getQuestItemsCount(DAIRY_OF_ALLANA) == 0
                and st.getQuestItemsCount(LIZARD_CAPTAIN_ORDER) == 0
                and st.getQuestItemsCount(HALF_OF_DAIRY) == 0
            ):
                if st.getInt("cond"):
                ...
                if (
                    st.getQuestItemsCount(MONEY_OF_SWINDLER) == 1
                    and st.getQuestItemsCount(DAIRY_OF_ALLANA) == 1
                    and st.getQuestItemsCount(LIZARD_CAPTAIN_ORDER) == 1
                    and st.getQuestItemsCount(HALF_OF_DAIRY) == 0
                ):
            ...
            if (
                ...
                and st.getQuestItemsCount(DAIRY_OF_ALLANA) == 0
                and st.getQuestItemsCount(LIZARD_CAPTAIN_ORDER) == 1
                and st.getQuestItemsCount(HALF_OF_DAIRY) == 1
            ):
                ...
                if (
                    ...
                    and st.getQuestItemsCount(LIZARD_CAPTAIN_ORDER) == 1
                    and st.getQuestItemsCount(HALF_OF_DAIRY) == 0
                    and st.getQuestItemsCount(DAIRY_OF_ALLANA)
                ):
                    htmltext = "30424-05.htm"
        elif (
            npcId == 30428
            and st.getInt("cond")
            and st.getQuestItemsCount(CRYSTAL_MEDALLION)
            and st.getQuestItemsCount(LIZARD_CAPTAIN_ORDER)
        ):
