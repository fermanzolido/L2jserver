...
class Quest(JQuest):
        ...
        if event == "1":
            ...
            if st.getInt("cond") == 0:
                if (
                    level >= 19
                    and classId == 0x26
                    and st.getQuestItemsCount(JEWEL_OF_DARKNESS) == 0
                ):
                    st.set("cond", "1")
                    st.setState(STARTED)
                    ...
                    level >= 19
                    and classId == 0x26
                    and st.getQuestItemsCount(JEWEL_OF_DARKNESS) == 1
                ):
            ...
            elif st.getQuestItemsCount(SEEDS_OF_LUNACY) == 0 and st.getQuestItemsCount(
                SEEDS_OF_DESPAIR
            ):
    ...
    def onTalk(self, npc, player):
        ...
        if npcId == 30421 and st.getInt("cond") == 0:
            ...
            if (
                st.getQuestItemsCount(SEEDS_OF_DESPAIR)
                and st.getQuestItemsCount(SEEDS_OF_HORROR)
                and st.getQuestItemsCount(SEEDS_OF_LUNACY)
                and st.getQuestItemsCount(SEEDS_OF_ANGER)
            ):
                ...
                st.getQuestItemsCount(SEEDS_OF_DESPAIR) == 1
                and st.getQuestItemsCount(FAMILYS_ASHES) == 0
                and st.getQuestItemsCount(LUCKY_KEY) == 0
                and st.getQuestItemsCount(CANDLE) == 0
                and st.getQuestItemsCount(HUB_SCENT) == 0
                and st.getQuestItemsCount(KNEE_BONE) == 0
                and st.getQuestItemsCount(HEART_OF_LUNACY) == 0
            ):
                htmltext = "30421-17.htm"
            elif (
                st.getQuestItemsCount(SEEDS_OF_DESPAIR) == 1
                and st.getInt("id") == 1
                and st.getQuestItemsCount(SEEDS_OF_ANGER) == 0
            ):
            ...
            if (
                st.getQuestItemsCount(HUB_SCENT) == 0
                and st.getQuestItemsCount(HEART_OF_LUNACY) == 0
            ):
                htmltext = "30419-01.htm"
                st.giveItems(HUB_SCENT, 1)
            elif (
                st.getQuestItemsCount(HUB_SCENT)
                and st.getQuestItemsCount(HEART_OF_LUNACY) < 3
            ):
            ...
            if (
                ...
                st.getQuestItemsCount(SEEDS_OF_DESPAIR) == 1
                and st.getQuestItemsCount(FAMILYS_ASHES) < 3
                and st.getQuestItemsCount(LUCKY_KEY) == 1
            ):
            ...
            npcId == 30415
            and st.getInt("cond") == 1
            and st.getQuestItemsCount(SEEDS_OF_ANGER) == 1
        ):
            htmltext = "30415-06.htm"
        elif (
            npcId == 30418
            and st.getInt("cond") > 0
            and st.getQuestItemsCount(SEEDS_OF_HORROR) == 0
        ):
            if (
                st.getQuestItemsCount(SEEDS_OF_DESPAIR) == 1
                and st.getQuestItemsCount(CANDLE) == 0
                ...
                st.getQuestItemsCount(SEEDS_OF_DESPAIR) == 1
                and st.getQuestItemsCount(CANDLE) == 1
                and st.getQuestItemsCount(KNEE_BONE) < 2
            ):
    ...
    def onKill(self, npc, player, isPet):
        ...
        if npcId == 20015:
            ...
            if (
                st.getInt("cond") == 1
                and st.getQuestItemsCount(HUB_SCENT) == 1
                and st.getQuestItemsCount(HEART_OF_LUNACY) < 3
            ):
                if st.getRandom(2) == 0:
