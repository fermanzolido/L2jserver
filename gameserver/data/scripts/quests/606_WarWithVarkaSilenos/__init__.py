    ...
    def onEvent(self, event, st):
        ...
        if event == "31370-03.htm":
            if (
                ...
                and st.getPlayer().getAllianceWithVarkaKetra() >= 1
            ):  # the alliance check is only temporary, should be done on core side/AI
                st.set("cond", "1")
                st.set("id", "1")
                st.setState(STARTED)
        ...
        if st:
            if st.getState() == STARTED:
                ...
                if npcId in Varka_Mobs and partyMember.getAllianceWithVarkaKetra() >= 1:
                    ...
                    if not st2:
                        numItems, chance = divmod(
                            Chance[npcId] * Config.RATE_DROP_QUEST, 1000
                        )
