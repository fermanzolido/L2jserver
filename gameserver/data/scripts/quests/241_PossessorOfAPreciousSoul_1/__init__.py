...
class Quest(JQuest):
    ...
    def onEvent(self, event, st):
        ...
        if event == "31272-5.htm":
            if (
                st.getPlayer().isProcessingRequest()
                or st.getPlayer().isProcessingTransaction()
            ):
                st.getPlayer().sendMessage("Another transaction in progress..")
            elif (
                cond == 20
                and st.getQuestItemsCount(LUNARGENT) >= 5
                and st.getQuestItemsCount(HELLFIRE_OIL)
                and st.getPlayer().isSubClassActive()
            ):
        ...
        if player.isSubClassActive():
            if npcId == TALIEN:
                ...
                if cond == 19:
                    ...
                    cond == 20
                    and st.getQuestItemsCount(LUNARGENT) < 5
                    and not st.getQuestItemsCount(HELLFIRE_OIL)
                ):
    ...
    def onKill(self, npc, player, isPet):
        ...
        if npcId == BARAHAM:
            ...
            if partyMember:
                ...
                if (
                    MALRUK_SUCCUBUS_CLAW_CHANCE >= chance
                    and st.getQuestItemsCount(MALRUK_SUCCUBUS_CLAW) < 10
                ):
            ...
            if partyMember:
                ...
                chance = st.getRandom(100)
                if (
                    CRIMSON_MOSS_CHANCE >= chance
                    and st.getQuestItemsCount(CRIMSON_MOSS) < 5
                ):
