    ...
    def onAdvEvent(self, event, npc, player):
        ...
        if event == "31371-04.htm":
            if (
                ...
                and st.getPlayer().getAllianceWithVarkaKetra() >= 2
            ):
                st.set("cond", "1")
                st.set("id", "2")
                st.set("aggro", "0")
    ...
    def onAttack(self, npc, player, damage, isPet):
        ...
        if st:
            if st.getState() == STARTED:
                ...
                if st.getInt("spawned") == 0 and npc.getObjectId() != st.getInt(
                    "npcid"
                ):
                    if npcId in Varka_Mobs:
                        if id > 2:
                            ...
                            AutoChat(
                                spawnedNpc, "You cannot escape Asefa's eyes!"
                            )  # this is only a temp message until we find out what it actually is! string = 61503
                            st.startQuestTimer(
                                "Asefa's Eye has despawned", 10000, spawnedNpc
                            )
                            if Red_Totem:
                                st.takeItems(Totem, -1)
        return
