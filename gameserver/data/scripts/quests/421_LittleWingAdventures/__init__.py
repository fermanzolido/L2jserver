...
error_3 = (
    "<html><body>Sage Cronos:<br>You need to be level "
    + str(MIN_PLAYER_LEVEL)
    + " to complete this quest.</body></html>"
)
error_4 = (
    "<html><body>Sage Cronos:<br>Your pet need to be level "
    + str(MIN_PET_LEVEL)
    + " to complete this quest.</body></html>"
)
...
qston_1 = (
    '<html><body>Sage Cronos:<br>So, you want to turn your hatchling into a more powerful creature?<br><br><a action="bypass -h Quest '
    + str(QUEST_NUMBER)
    + "_"
    + QUEST_NAME
    + ' 16">Yes, please tell me how</a><br></body></html>'
)
...
ftm_11 = (
    "<html><body>Fairy Tree of Wind: <br>I'll let your hatchling drink from my sap, but you will have to cover the wound your pet will do on me with one of these leafs you have, they are hypoallergenic<br><br><a action=\"bypass -h Quest "
    + str(QUEST_NUMBER)
    + "_"
    + QUEST_NAME
    + " 1\">It's ok</a></body></html>"
)
ftm_12 = "The hatchling has drunk the sap of the fairy tree of the wind."
ftm_21 = (
    '<html><body>Fairy Tree of Star: <br>Oh! One of those nasty ghosts hurted my bark... look! Only a fairy leaf could cure my wound... <br><br><a action="bypass -h Quest '
    + str(QUEST_NUMBER)
    + "_"
    + QUEST_NAME
    + ' 2">Give it a leaf</a></body></html>'
)
ftm_22 = "The hatchling has drunk the sap of the fairy tree of the star."
ftm_31 = (
    "<html><body>Fairy Tree of Twilight: <br>Ok, i do know the way this is supposed to be, but we don't have the time to wait your hacthling to hit me for hours... Let's make it quick<br><br><a action=\"bypass -h Quest "
    + str(QUEST_NUMBER)
    + "_"
    + QUEST_NAME
    + ' 4">Fine, take the leaf</a></body></html>'
)
ftm_32 = "The hatchling has drunk the sap of the fairy tree of the twilight."
ftm_41 = (
    '<html><body>Fairy Tree of Abyss: <br>That your pet will bite me and you gonna put a leaf in my wound? No way! No! Wait!... argh... if i could run like Black Willows do...<br><br><a action="bypass -h Quest '
    + str(QUEST_NUMBER)
    + "_"
    + QUEST_NAME
    + ' 8">Say sorry</a></body></html>'
)
...
FAIRY_TREES = [
    [27185, 1, 113356, 93848, -2072, ftm_11, ftm_12],
...
class Quest(JQuest):
    ...
    def onTalk(self, npc, player):
        ...
        if player.getPet() == None:
            htmltext = error_1
            st.exitQuest(1)
        elif player.getPet().getTemplate().npcId not in [
            12311,
            12312,
            12313,
        ]:  # npcIds for hatchlings
            htmltext = error_5
            st.exitQuest(1)
            ...
            if id == STARTING:
                ...
                item = CONTROL_ITEMS[
                    player.getInventory()
                    .getItemByObjectId(player.getPet().getControlItemId())
                    .getItemId()
                ]
                player.getPet().deleteMe(
                    player
                )  # both despawn pet and delete controlitem
                st.giveItems(item, 1)
                st.exitQuest(1)
            ...
            if 0 < leafs:
                for i in range(4):
                    if npcId == FAIRY_TREES[i][0] and (
                        st.getInt("id") | FAIRY_TREES[i][1] != st.getInt("id")
                    ):
                        for j in range(2):
                            for k in range(2):
                                st.addSpawn(
