...
class benom(JQuest):
        ...
        if castleOwner > 0:
            ...
            if (siegeDate - System.currentTimeMillis()) > 0:
                self.startQuestTimer(
                    "BenomRaidRoomSpawn", benomRaidRoomSpawn, None, None
                )
    ...
    def onAdvEvent(self, event, npc, player):
        ...
        if event == "BenomTeleSpawn":
            self.Teleport = self.addSpawn(
                BenomTeleport, 11013, -49629, -547, 13400, False, 0
            )
            ...
            if self.BenomWalkRouteStep == 33:
                ...
                npc.getAI().setIntention(
                    CtrlIntention.AI_INTENTION_MOVE_TO, L2CharPosition(X, Y, Z, 0)
                )
            ...
            if Rnd.get(100) < 40:
                npc.broadcastPacket(
                    CreatureSay(npc.getObjectId(), 0, "Benom", BenomSpeak[Rnd.get(4)])
                )
