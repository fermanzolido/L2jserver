...
class turek_orc_warlord(JQuest):
    ...
    def onAttack(self, npc, player, damage, isPet):
        ...
        if self.FirstAttacked:
            if Rnd.get(40):
                return
            npc.broadcastPacket(
                CreatureSay(objId, 0, npc.getName(), "You wont take me down easily.")
            )
        else:
            self.FirstAttacked = True
            npc.broadcastPacket(
                CreatureSay(objId, 0, npc.getName(), "The battle has just begun!")
            )
        return
    ...
    def onKill(self, npc, player, isPet):
        ...
        if npcId == self.turek_orc_warlord:
            ...
            self.addSpawn(
                npcId, npc.getX(), npc.getY(), npc.getZ(), npc.getHeading(), True, 0
            )
        return
