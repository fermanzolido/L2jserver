...
class squash(JQuest):
    ...
    def onSkillUse(self, npc, player, skill):
        ...
        if skillId != SKILL_NECTAR:
            return
        if npcId not in WATERED_SQUASH:
            return
                ...
                if npc.getGood() >= 3:
                    if npcId == 12774:
                        ...
                        self.startQuestTimer(
                            "Good By", 120000, newGourd, player
                        )  # Через 2 минуты исчезновение
                        self.startQuestTimer(
                            "Good By2", 90000, newGourd, player
                        )  # 30 секунд до исчезновения
                        self.startQuestTimer(
                            "Good By3", 100000, newGourd, player
                        )  # 20 секунд до исчезновения
                        self.startQuestTimer(
                            "Good By4", 110000, newGourd, player
                        )  # 10 секунд до исчезновения
                        mytext = [
                        ...
                        newGourd.setOwner(player.getName())
                        self.startQuestTimer(
                            "Good By1", 120000, newGourd, player
                        )  # Через 2 минуты исчезновение
                        self.startQuestTimer(
                            "Good By2", 90000, newGourd, player
                        )  # 30 секунд до исчезновения
    ...
    def onAttack(self, npc, player, damage, isPet):
        ...
        if npcId in self.adultLargeSquash:
            if Rnd.get(30) < 2:
                ...
                npc.broadcastPacket(
                    CreatureSay(objId, 0, npc.getName(), mytext[Rnd.get(len(mytext))])
                )
        return
