...
def refactor_python_code(project_path):
    ...
    try:
        ...
        subprocess.run(
            [sys.executable, "-m", "lib2to3", "-w", "."], cwd=project_path, check=True
        )
...
def git_create_branch(project_path, branch_name):
    ...
    try:
        ...
        if result.stdout.strip():
            ...
            subprocess.run(
                ["git", "checkout", branch_name], cwd=project_path, check=True
            )
        else:
            subprocess.run(
                ["git", "checkout", "-b", branch_name], cwd=project_path, check=True
            )
...
def git_add_commit_push(project_path, message, branch="main"):
    ...
    try:
        ...
        subprocess.run(
            ["git", "push", "-u", "origin", branch], cwd=project_path, check=True
        )
...
def main():
    ...
    project_path = eval(input("Introduce la ruta del proyecto: "))
